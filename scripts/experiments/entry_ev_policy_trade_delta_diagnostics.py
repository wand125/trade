#!/usr/bin/env python3
"""Compare two entry-EV policy replay trade directories."""
...
def build_policy_delta(
    *,
    ...
    base_monthly = filter_monthly(
        read_monthly_metrics(base_run),
        candidates=candidates,
        families=families,
        months=months,
    )
    candidate_monthly = filter_monthly(
        read_monthly_metrics(candidate_run),
        candidates=candidates,
        families=families,
        months=months,
    )
    candidate_lookup = {monthly_key(row): row for _, row in candidate_monthly.iterrows()}
    missing = [key for _, row in base_monthly.iterrows() if (key := monthly_key(row)) not in candidate_lookup]
    if missing:
        preview = ", ".join("/".join(key) for key in missing[:5])
        suffix = "" if len(missing) <= 5 else f" ... ({len(missing)} missing)"
        raise ValueError(f"candidate monthly rows missing: {preview}{suffix}")

    frames: list[pd.DataFrame] = []
    for _, base_row in base_monthly.iterrows():
        key = monthly_key(base_row)
        candidate_row = candidate_lookup[key]
        base_trades = read_month_trades(base_run, base_row)
        candidate_trades = read_month_trades(candidate_run, candidate_row)
...
def run_policy_delta(args: argparse.Namespace) -> Path:
    ...
    for name, columns in group_specs.items():
        summaries[name] = write_summary(delta, run_dir, group_name=name, group_columns=columns)
    ...
    for name, columns in blocking_specs.items():
        write_blocking_summary(delta, run_dir, group_name=name, group_columns=columns)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--label", default="entry_ev_policy_trade_delta_diagnostics")
    return parser


def main(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    run_policy_delta(args)
    return 0
