#!/usr/bin/env python3
"""Attach direction-side inversion risk to prediction rows."""
...
DEFAULT_SCORE_KIND_PREFIX = "direction_inversion_bucket"
...
def normalize_common_targets(path: Path, *, target: str) -> pd.DataFrame:
    frame = pd.read_csv(path)
    required = {"month", target, *GROUP_COLUMNS}
    missing = sorted(required - set(frame.columns))
    if missing:
        raise ValueError(f"common target frame missing columns: {', '.join(missing)}")
    output = frame.copy()
    ...
    if "candidate_adjusted_pnl" in output.columns:
        output["candidate_adjusted_pnl"] = numeric_series(output, "candidate_adjusted_pnl")
    return output
...
def build_side_rows(
    ...
    *,
    ...
    missing = [
        column
        for column in [risk_column, support_column, pressure_column]
...
def apply_direction_inversion_risk(
    ...
    *,
    ...
    for month, group in side_rows.groupby("month", dropna=False, sort=True):
        ...
        if not np.isfinite(global_rate):
            output["predicted_direction_inversion_risk"] = np.nan
            output["direction_inversion_prediction_support"] = 0
            output["direction_inversion_prediction_source"] = "no_prior"
        elif table.empty:
            output["predicted_direction_inversion_risk"] = global_rate
            output["direction_inversion_prediction_support"] = 0
            output["direction_inversion_prediction_source"] = "global"
        else:
            merged = output.merge(table, how="left", on=GROUP_COLUMNS)
            bucket_hit = merged["predicted_target_rate"].notna()
            merged["predicted_direction_inversion_risk"] = merged[
                "predicted_target_rate"
            ].where(bucket_hit, global_rate)
            merged["direction_inversion_prediction_support"] = pd.to_numeric(
                merged["prediction_support"],
                errors="coerce",
            ).fillna(0.0).astype(int)
            merged["direction_inversion_prediction_source"] = np.where(
                bucket_hit,
                "bucket",
                "global",
            )
...
def add_side_direction_inversion_columns(
    predictions: pd.DataFrame,
    *,
    targets: pd.DataFrame,
    target: str,
    ...
    for side in ["long", "short"]:
        ...
        side_rows = apply_direction_inversion_risk(
            side_rows,
            targets,
            target=target,
            prior_strength=prior_strength,
        ...
        for column in [
            ...
            "predicted_direction_inversion_risk",
            "direction_inversion_prediction_support",
            "direction_inversion_prediction_source",
        ]:
            output[f"pred_direction_inversion_{side}_{column}"] = side_rows[column].to_numpy()
    return output


def add_direction_inversion_adjusted_scores(
    predictions: pd.DataFrame,
    *,
    family: str,
        ...
        summary_rows.append(
            {
                ...
                "use_global_risk": bool(use_global_risk),
...
def direction_risk_distribution(
    ...
    *,
    ...
    for side in ["long", "short"]:
        risk = numeric_series(
            enriched,
            f"pred_direction_inversion_{side}_predicted_direction_inversion_risk",
            default=np.nan,
        )
        source = enriched[
            f"pred_direction_inversion_{side}_direction_inversion_prediction_source"
        ].astype(str)
        rows.append(
            {
                "family": family,
...
def target_calibration_summary(
    ...
    *,
    ...
    for month in sorted(targets["month"].astype(str).unique()):
        ...
        predicted = apply_direction_inversion_risk(
        ...
        scores = numeric_series(predicted, "predicted_direction_inversion_risk", default=np.nan)
...
def build_policy_inputs(args: argparse.Namespace) -> Path:
    ...
    targets = normalize_common_targets(args.common_targets, target=args.target)
    ...
    for family, prediction_path in family_predictions.items():
        ...
        enriched = add_side_direction_inversion_columns(
            predictions,
            targets=targets,
            target=args.target,
        ...
        enriched, family_summary = add_direction_inversion_adjusted_scores(
            enriched,
            family=family,
        ...
        distribution_rows.extend(direction_risk_distribution(enriched, family=family))
        output_path = enriched_dir / f"{family}_predictions_direction_inversion.parquet"
    ...
    risk_distribution.to_csv(run_dir / "direction_inversion_risk_distribution.csv", index=False)
    calibration.to_csv(run_dir / "direction_inversion_target_calibration.csv", index=False)
    config = {
        "family_predictions": family_predictions,
        "common_targets": args.common_targets,
        "target": args.target,
    ...
    print("Direction inversion risk distribution:")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--common-targets", type=Path, required=True)
    ...
    parser.add_argument("--penalty-strengths", default="0.25,0.5")
    ...
    parser.add_argument("--label", default="entry_ev_direction_inversion_policy_inputs")
    return parser
