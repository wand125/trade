#!/usr/bin/env python3
"""Chronological large-loss head diagnostics for selected entry-EV trades."""
...
def normalize_prior_rows(
    ...
    *,
    ...
    return output.sort_values(["supervised_target_mode", "group_spec", "month"]).reset_index(
        drop=True
    )
...
def train_predict_fold(
    ...
    *,
    ...
    y_train = train["is_large_loss"].astype(int)
...
def chronological_predictions(
    ...
    *,
    ...
    for (mode, group_spec), group in frame.groupby(group_columns, dropna=False):
        ...
        for feature_set_name, numeric_features in feature_sets.items():
            for month in months:
                ...
                if can_train:
                    ...
                    default = float(train["is_large_loss"].mean()) if len(train) else 0.0
                    preds = np.full(len(test), default, dtype=float)
                    model_used = False
                scored = test.copy()
                scored["large_loss_feature_set"] = feature_set_name
                scored["pred_large_loss_prob"] = preds
                scored["large_loss_model_used"] = bool(model_used)
                scored["large_loss_train_rows"] = int(len(train))
                scored["large_loss_train_months"] = train_months
                ...
                fold_rows.append(
                    {
                        ...
                        "positive_train_rows": int(train["is_large_loss"].sum())
                        if len(train)
                        else 0,
                        "model_used": bool(model_used),
                        "target_large_loss_rate": float(test["is_large_loss"].mean()),
...
def score_summary(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, group in frame.groupby(
        ["supervised_target_mode", "group_spec", "large_loss_feature_set"],
        dropna=False,
    ):
        mode, group_spec, feature_set = keys
        y = group["is_large_loss"].astype(int)
        ...
        rows.append(
            {
                ...
                "large_loss_count": int(y.sum()),
                "large_loss_rate": float(y.mean()) if len(y) else 0.0,
                ...
                "model_used_rate": float(group["large_loss_model_used"].mean())
                if len(group)
                else 0.0,
            }
        )
...
def summarize_flagged(frame: pd.DataFrame, mask: pd.Series) -> dict[str, Any]:
    ...
    return {
        ...
        "flagged_large_loss_precision": float(len(flagged_large) / len(flagged))
        if len(flagged)
        else 0.0,
...
def build_diagnostics(args: argparse.Namespace) -> Path:
    ...
    predictions.to_csv(run_dir / "selected_trade_large_loss_head_predictions.csv", index=False)
    folds.to_csv(run_dir / "selected_trade_large_loss_head_folds.csv", index=False)
    score.to_csv(run_dir / "selected_trade_large_loss_head_score_summary.csv", index=False)
    thresholds.to_csv(
        run_dir / "selected_trade_large_loss_head_threshold_summary.csv",
        index=False,
    )
    worst.to_csv(run_dir / "selected_trade_large_loss_head_worst_rows.csv", index=False)
    config = {
    ...
    print(f"Wrote large-loss head diagnostics to {run_dir}")
    print("\nScore summary:")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--label", default="entry_ev_selected_trade_large_loss_head")
    parser.add_argument("--output-dir", type=Path, default=Path("data/reports/backtests"))
    parser.add_argument("--target-modes", default="factor,pnl")
    parser.add_argument(
        "--group-specs",
        default="direction,combined_regime,session_regime",
    )
