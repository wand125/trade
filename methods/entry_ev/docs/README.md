...
ML によるエントリー期待値予測研究(XAUUSD 短期)のドキュメント置き場。パスはすべてリポジトリルート基準で表記する。
...
2. `methods/entry_ev/docs/summary/`
...
5. 直近の実験がある場合は `methods/entry_ev/docs/reports/` の最新レポートを読む。最新判断はファイルシステムの更新時刻(mtime)や `更新日時` ではなく、ファイル内の `日時` を基準にする。通し番号はその `日時` 順に由来する補助情報として扱う。
