...
`methods/entry_ev/docs/reports/` を個別に読む前の研究地図。番号はレポート本文の `日時:` 順に由来する。
...
| `00375` | Broad support abstention stability | support-sufficient target条件を緩め13 target評価へ拡張。abstention gateは壊れなかったが介入は同じ1件だけで、複数target有効性は未確認。 |
...
| `00380` | Surface target outcome diagnostics | 00378 surfaceをtarget単位に分類。非oracle ev_ge5 + prior_actual_meanは修復成功1、candidate gap 3、risk gap 1で、制約通過をpolicy readinessと読まない。 |
| `00381` | Outcome-constrained surface | 00380分類をselector surface本体へ統合。00378再実行では旧winner-damage pass 8/16、新target outcome pass 0/16。 |
| `00382` | All-family shrunk prior surface | all-family prior + shrunk priorをsurfaceへ接続。candidate gapは少し減るがtarget outcome passは0/48で、direct policyはreject。 |
| `00383` | Surface support gap diagnostics | 00382のcandidate gapを分解。hgb/fresh03はpoolもpositive actual candidateもあるが、prior month最大1かつprior actual mean負なのでsupport候補にならない。 |

## Current Clusters

| Cluster | Key reports | What to remember |
|---|---|---|
| Latest decision | `00383` | candidate gapはraw pool不足ではなく `prior_month_and_actual_gap`。early-month targetは薄くて負のprior候補をどう扱うかの診断へ進む。標準policyはNoTrade。 |
...
118. `00375_2026-07-03_entry_ev_broad_support_abstention_stability.md`
...
123. `00380_2026-07-03_entry_ev_surface_target_outcome.md`
124. `00381_2026-07-03_entry_ev_outcome_constrained_surface.md`
125. `00382_2026-07-03_entry_ev_all_family_shrunk_prior_surface.md`
126. `00383_2026-07-03_entry_ev_surface_support_gap.md`
...
```text
Report: 00383 Entry EV Surface Support Gap Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: 00382に残るcandidate gapはどのsupport filterで落ちているか
Best evidence: hgb2024 2024-03 / fresh2024 2024-03はcandidate poolとpositive actual candidateがあるが、max prior month count 1、max prior actual mean -1.3401でsupport落ち。
Decision: support gap decompositionはaccepted。early-month targetはraw candidate生成なしではなく、薄くて負のprior候補の扱いが課題。
Next: prior_month_count=1候補をpolicyではなくsupport-limited scoring reportで診断する
```
