...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "experiments"
    / "entry_ev_policy_trade_delta_diagnostics.py"
)
SPEC = importlib.util.spec_from_file_location(
    "entry_ev_policy_trade_delta_diagnostics",
    SCRIPT_PATH,
)
entry_ev_policy_trade_delta_diagnostics = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
sys.modules[SPEC.name] = entry_ev_policy_trade_delta_diagnostics
SPEC.loader.exec_module(entry_ev_policy_trade_delta_diagnostics)


def write_policy_run(root: Path, trades: pd.DataFrame) -> None:
    ...
    ).to_csv(root / "monthly_policy_metrics.csv", index=False)
    trade_dir = root / "trades" / "toy" / candidate
...
class EntryEvPolicyTradeDeltaDiagnosticsTests(unittest.TestCase):
    def test_run_policy_delta_summarizes_added_removed_and_common_trades(self):
        candidate = "q99_sg95_rank90_floor5_side_regime_session_month"
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            base_run.mkdir()
            candidate_run.mkdir()
            write_policy_run(base_run, base_trades)
            write_policy_run(candidate_run, candidate_trades)
            ...
            args = argparse.Namespace(
                ...
                output_dir=tmp_path,
                label="unit_policy_delta",
            )
            run_dir = entry_ev_policy_trade_delta_diagnostics.run_policy_delta(args)
