...
次足実体を判定時ATRで正規化してtrain sample weightへ使う方式は `--train-weighting body_atr` で再現できる。未来の次足実体は教師重みにだけ使用し、入力特徴・calibration・test推論には使用しない。weighted HGB単体は方向精度が悪化したため不採用だが、HGB方向を維持した25% confidence blendは `config/m15_body_atr_weighted_confidence_candidate_v1.json` のconfidence 0.54精度重視候補として固定した。定義を変えないM1移植では通常25% blendがbaselineのaccuracyを7/7、Brier/log lossを6/7fold改善し、方向維持0.515もconfirmation accuracy・scoreを改善した。しかし方向はDirectional-Clarity/Distribution Shift、confidenceはDisagreementが上回ったためM1では再現専用とする。同じ固定仕様のM30単体はbaseline比all -145件、通常25% blendも-27件・accuracy 3/7foldだった。Directional-Clarity通常blendよりall -59件で、日次bootstrapはClarityのproper score優位を支持した。方向維持0.515はconfirmation accuracy・coverage・scoreが反転し、Pressure 0.52にaccuracy 0/7、Pressureとの固定平均も既存Pressure + AR 0.55を超えないためM30も再現専用とする。標準学習は `--train-weighting uniform` のままである。

方向0/1教師と全行を維持しつつ次足の方向明瞭度をsample weightへ使う場合は `--train-weighting directional_clarity` を指定する。重みは平均1へ正規化した `0.5 + abs(next body) / next range` で、未来rangeは特徴へ入れない。M15方向維持0.525はdevelopment/confirmationを改善したが、Signed-body Quantile/Clear-bodyよりaccuracy・selection score・fold安定性が低いため再現専用とする。定義を変えないM1通常25% blendはbaselineのaccuracy/Brier/log lossを7/7fold改善し、all +868件、日次bootstrapも支持した。ただしPathとは精度同等、Distribution Shiftとは精度9件差でproper scoreが有意に悪く、Extra Treesとは3件差だったためM1でも新規forward役割へ追加しない。方向維持0.51もconfirmationで反転した。同じ固定仕様のM30単体はbaseline比development +66件、confirmation +77件、all +143件だったが、accuracy 4/7fold、all日次区間は-0.0673〜+0.4725ptだった。現行Haar入り候補との固定50/50方向平均はbaseline比+160件・5/7foldかつproper scoreを改善した一方、親候補比は+28件・3/7foldで直接差を確定できないため候補を増やさない。方向維持0.51はconfirmationで反転し、Pressure 0.52にaccuracy 0/7、Pressureとの0.55固定平均もPressure + ARに2/7だったため、M30も再現専用とする。

次足の実体/range比率に、方向側の高安へ終値が到達した割合を掛けた教師品質を使う場合は `--train-weighting directional_follow_through` を指定する。raw weightは `0.5 + clarity * direction_aligned_close_location`、0.5〜1.5、train内平均1で、未来OHLCは教師重みにだけ使う。M30単体はbaseline比all +112件、confirmation +90件で、日次bootstrapはBrier/log loss改善を支持したがaccuracy差は未確定だった。通常25%方向blendは-5件、Clarity・Extra Trees・Haar方向候補への直接差も未確定のため方向には採用しない。Haar方向候補との固定50/50平均0.55はPressure + ARよりall coverage +0.902pt、accuracy +0.514pt、selection score +0.002451、score 6/7foldで、development/all scoreのbootstrap区間も改善した。confirmation scoreはほぼ同値で局所セルが疎いため、`config/m30_haar_directional_follow_through_confidence_shadow_v1.json` の非認可forward shadowに限定し、fair odds・運用policyを変更しない。固定M1移植では通常25%方向blendがbaselineのaccuracy/Brier/log lossを7/7fold、方向維持0.515がaccuracy/scoreを6/7、proper scoreを7/7fold改善した。しかし方向はPath/Shift/Clarity、confidenceはTransition guard/Disagreement/Shiftを超えず、0.55 confirmationは119件、0.575は0件だったためM1では再現専用とする。
...
同じ固定SpectralをM5へ移植すると、単体はbaseline比-58件、通常25%方向blendも-11件だった。方向維持0.515はconfirmation accuracyだけbootstrap改善側でもcoverage低下が確定し、selection score/proper scoreは未確定、Profile/Haarへ年別3/7対4/7だった。0.55もFollow-throughよりaccuracy・coverage・score・proper scoreが低く、Profileとの固定50/50 confidence平均もconfirmationと年別を下げた。M5でも再現専用とし、既存候補・fair odds・policyを変更しない。

M30 Distribution Shiftの方向維持75/25・confidence 0.52 coverage challengerは、保存済みWindows artifactによるfull runtime parityを完了した。latest `p(up)=0.5299834177`は固定閾値を通過したがfresh edgeではなく、Pressure 0.52、Pressure + AR 0.55、authoritative confidence、fair odds、policyは変更しない。

M15 Distribution Shiftの方向維持75/25・confidence 0.515 broad候補も、保存済みWindows artifactによるfull runtime parityを完了した。latest `p(up)=0.5566218193`は固定閾値を通過したがfresh edgeではなく、同一platform registry再構築とfresh calibrationまでauthoritative confidence、fair odds、policyは変更しない。

M5 EWMA Asymmetryの方向維持75/25・confidence 0.515 broad候補は、候補model単体latestとは別に実際のblendでfull runtime parityを完了した。latest `p(up)=0.5249062717`は固定閾値を通過したがfresh edgeではなく、Profile系候補、authoritative confidence、fair odds、policyは変更しない。

M1 Path Persistenceの通常75/25方向候補は、保存済みWindows artifactによるfull runtime parityを完了した。latestはbaselineとPathが方向不一致でもblendはdown、`p(up)=0.4986377202`となった。fresh edgeではなく、authoritative direction/confidence、fair odds、policyは変更しない。

M1 Distribution Shiftの通常75/25方向・方向維持0.51 confidence候補も、保存済みWindows artifactによるfull runtime parityを完了した。latestは両source down、`p(up)=0.4974296583`で0.51 lane非選択。旧platform値との差を記録してWindows値へ固定したがfresh edgeではなく、Path、authoritative direction/confidence、fair odds、policyは変更しない。

M1 Five-model Disagreement 0.515は、固定sourceのExtra Trees/LightGBM/causal TCN生modelが取得できないためruntime parityを保留する。baseline/Pathだけの近似やparity目的の再学習は行わず、研究上のbalanced confidence候補だけを維持する。

M5 Haar Multiscaleの方向維持75/25・confidence 0.515 broad候補は、保存済みWindows artifactによるfull runtime parityを完了した。latest `p(up)=0.5221144187`は固定閾値を通過したがfresh edgeではなく、既存候補、authoritative confidence、fair odds、policyは変更しない。

M5 Intrabar Profileの方向維持75/25・confidence 0.515 broad基準候補も、保存済みWindows artifactによるfull runtime parityを完了した。latest `p(up)=0.5224558998`は固定閾値を通過したがfresh edgeではなく、authoritative confidence、fair odds、policyは変更しない。
...
同じ18列を定義・3/8/21本・HGB/Platt・25% weightを変えずM5へ固定移植した。1,182,895完成M5の事前監査で完全重複はなかったが、range加重body pressureは既存baselineと最大0.83〜0.86相関だった。通常方向blendはbaseline比all +36件・accuracy 4/7foldでも区間0跨ぎで、既存Intrabar Pressureとの固定50/50平均はaccuracy 0/7、proper score悪化が確定した。方向維持0.515はconfirmationでbaseline/Profileへ反転し、Profile固定平均もconfirmation accuracy・coverage・scoreの悪化が確定した。Profile×TransitionとFollow-through 0.55も超えないためM5でも再現専用とし、window・subset・parameter・weight・閾値を同じ履歴で再探索しない。
...
直近16本×5加工系列を小型の因果TCNで学ぶ場合は `--feature-set tcn_sequence --model-type tcn` を使う。channelはATR正規化return/body/range、中心化close location、ATR正規化wick balanceで、生価格水準は含まない。各foldのtrainだけでchannel標準化し、2層1,073 parameterを8 epoch学習する。完全無変動足の0/0は値動き証拠なしの0へ定義し、sequence全列の有限0をテストする。M15では単体と通常方向blendを不採用、方向維持型0.52も既存signed-bodyより弱いため `config/m15_tcn_confidence_shadow_v1.json` に限定した。一方、同じ固定仕様のM1方向維持25% blendはdevelopment選択0.515でaccuracy 7/7、selection score 6/7、Brier/log loss各6/7foldを改善し、確認期間accuracy 52.509%→53.041%、score 0.006837→0.007506となったため `config/m1_tcn_confidence_candidate_v1.json` にselective forward候補として固定した。確認mean confidence 51.972%はaccuracyを1.070pt過小評価し、高閾値は疎なのでfair odds・policyには使わない。固定仕様のM5移植ではTCN単体と通常方向を不採用。TCN単独0.515も既存Profile/EWMAを超えなかったが、Profile confidenceとTCN confidenceの固定50/50平均0.515はbaseline比accuracy 7/7、score 6/7、proper score 5/7foldで、confirmation/allのaccuracy・score bootstrapも改善したため `config/m5_profile_tcn_confidence_shadow_v1.json` のsequence-diversity shadowへ固定した。親Profileとの差はaccuracy 5/7、score 4/7、全期間差区間0跨ぎなので置換しない。full runtime parityはreport 00164で達成したがfresh edgeではなく、完全未使用期間の親head-to-headと局所校正までauthoritative confidence・fair odds・policyへ使わない。

同じ系列を再帰状態で比較する場合は `--feature-set tcn_sequence --model-type causal_gru` を使う。単層GRU hidden 16は1,121 parameterで、TCNより48多い近似容量比較である。M1固定7foldでは通常25%方向blendがbaselineを全体+560件にしたが日次accuracy区間は0を跨ぎ、TCN blendと同等だった。方向維持0.515はbaselineを7/7fold改善したが、5-model Disagreementよりaccuracy・coverage・selection score・全期間proper scoreが下だった。GRUを6番目に等重み追加してもcoverage低下でselection scoreを改善しなかったため、GRUと6-model拡張は再現専用とし、既存TCN/5-model Disagreementを維持する。同じ仕様のM5通常25%方向blendはbaseline比+126件、accuracy 6/7fold、all Brier/log loss日次区間を改善したが、accuracy区間は0を跨ぎ、Pressureにはall +30件でも3/7fold・proper score点劣後だった。Profile x GRU固定50/50 confidence 0.515は親Profileへaccuracy/score各6/7でall accuracy区間を改善した一方、coverage低下でscoreは未確定、confirmation scoreは反転し、Profile x Transitionにはall accuracy/proper scoreが有意に劣った。M5も全用途を再現専用とし、Pressure/Profile/Profile x TCN/Profile x Transition/Follow-through、authoritative予測・fair odds・policyを変更しない。

現在方向、連続方向本数、直近反転率、短長volatility状態を階層ベイズ遷移表で直接学ぶ場合は `--feature-set direction_transition_state --model-type transition_bayes` を使う。135 encoded slot中81状態が構造的に到達可能で、全foldが81/81を観測した。prior strengthはstate 64、親256で固定する。M1単体と通常25%方向blendは既存Session/Pathを超えないため方向用途には使わない。方向維持0.515はTCNより高精度だがcoverage-aware scoreが低かったため、developmentで弱かったup×low volatilityだけを0.5 abstentionにする固定guardを確認期間で検証した。さらにguard済みconfidenceと5-model Disagreementをdevelopment選択50/50で合成すると、0.515 accuracyはTCNに7/7、score 6/7fold勝ち、全期間52.5827% / coverage 16.0178% / score 0.009674となった。`config/m1_transition_guard_disagreement_confidence_candidate_v1.json` のaccuracy specialistに固定し、Disagreementはcoverage Pareto challengerとして維持する。再構築registryでも50/50が0.515 broad-role champion、Disagreementがcoverage leader、TCNがdominatedとなった。confirmationは1.402pt過小評価し0.55が39件だけなのでfair odds・policyには使わない。同じ固定状態をM5へ移植すると単体はbaselineへaccuracy/Brier/log loss各0/7foldだったが、PressureとTransitionの通常方向blendを固定50/50平均した方向shadowはbaseline比accuracy 7/7、Pressure比all proper scoreの日次区間を改善した。ProfileとTransitionの方向維持confidenceを固定50/50平均した0.515はProfile比accuracy 7/7、all accuracy・selection score・proper scoreの区間、Profile×TCN比all accuracy・proper scoreの区間を改善したため `config/m5_transition_bayes_ensemble_shadow_v1.json` のaccuracy/proper-quality broad shadowへ固定する。coverageは各親より狭くconfirmation score差とdown×normal edgeは未確認である。full runtime parityはreport 00163で達成したが、fresh evidenceではないため既存Pressure/Profile/Profile×TCN、authoritative confidence・fair odds・policyを置換しない。0.55はFollow-throughへscore 0/7のため使わない。

M5 Transition shadowのfull runtime parityは保存済みwalk-forward modelを再学習せず `predict_latest_multi_ensemble.py` で生成する。`--source LABEL=MODEL_DIR=WEIGHT`を3回指定し、方向shadowはbaseline/Pressure/Transition=`0.75/0.125/0.125`、confidence shadowはbaseline/Profile/Transitionを同じ重みで`--preserve-first-direction`付きにする。CLIは7fold境界、学習・校正設定、latest timestamp、重み合計をguardし、source別確率とparity manifestを残す。runtime parityは達成済みだがfresh edgeではなく、oddsとstrict eligibilityは非認可のままとする。詳細はreport 00163。

同じCLIはDirectional Follow-throughの固定baseline/weighted=`0.75/0.25`方向維持runtimeにも使う。教師weightingの差だけは`--allow-config-difference train_weighting`で明示し、uniformとdirectional_follow_throughの値をparity manifestへ残す。その他の設定差は拒否する。最新confidenceは0.55未満で非選択、runtime parityは達成済みだがfresh evidenceではない。詳細はreport 00165。
...
OOSが非重複の複数directoryへ分割されている場合は、`--first-dir` または `--second-dir` を時系列setごとに反復する。readerがfold/timestamp重複を検証して結合するため、中間parquetへ手動結合しない。
...
指数減衰する方向・変動状態を加工する場合は `--feature-set ewma_asymmetry_state` を使う。半減期4/16/64のprior-volatility標準化return innovationとdrift/volatility、4対16・16対64のvolatility balance、半減期16/64のupside/downside energy balanceとlagged-return×current-variance leverage momentを固定12列へ圧縮する。全列は[-1,1]、価格scale不変、未来不参照、gapごとにEWMAをreset、flat全0をテストした。M5 Windows canonicalの方向維持25% confidence 0.515はbaseline比accuracy 5/7、selection score 6/7、Brier/log loss 7/7foldを改善し、confirmation accuracy +0.11681ptとscore +0.000681、全期間proper scoreの日次bootstrap区間も改善側だったため `config/m5_ewma_asymmetry_confidence_candidate_v1.json` のparallel broad候補に固定する。Profileとはall accuracy/score差が未確定、固定50/50はconfirmationを下げたため独立維持する。0.55は最新foldが47.60%へ崩れ、down-normal 0.515も局所edge未確認なので、Follow-through 0.55、authoritative confidence、fair odds、売買policyを変更しない。全時間足への固定移植では、M1 0.51がbaselineへaccuracy/score各7/7fold、日次区間も改善したが、現行Shiftにall accuracy -0.05302pt、score -0.000287、proper scoreも有意に劣った。M15 0.515もShiftへ全主指標で点劣後し、M30 0.52とShiftとの固定50/50平均はShiftにaccuracy・score各1/7foldだった。M1/M15/M30では再現専用とし、各Shift候補を維持する。EWMAの採用はM5固有であり、時間足共通ruleにはしない。
...
CatBoostの再現には `--model-type catboost` を使う。Ordered boosting、symmetric depth 6、300 iteration、learning rate 0.03、L2 5を標準固定値とし、後続calibration期間のPlattは他学習器と共通である。共有計算機では `--catboost-thread-count 8` のように並列数も明示する（既定値 `-1` は全CPU）。M15方向はconfirmationで悪化し、方向維持0.525 confidenceも既存候補を超えない。定義を変えないM1通常25%方向blendはbaselineに対し全体+490件、accuracy 6/7、Brier/log loss 7/7foldで、日次bootstrapも全体accuracyとproper scoreの改善を支持した。しかしPath/LightGBMにaccuracy 2/7対5/7、Volatility/Sessionのproper-score役割も超えず、M1 confidence 0.515はTCNにaccuracy 0/7対7/7で負けた。M1でもconfigを発行せず再現専用とする。M5固定移植では単体と通常25%方向blendがbaseline/Pressureへ悪化した。方向維持0.515はbaselineへのall accuracy・selection score・Brier/log loss日次区間を改善し、確認期間の局所校正も良好だったが、Profileへaccuracy 2/7、confirmation反転、coverageとall proper scoreの悪化区間が確定し、Profile×Transitionにはaccuracy 1/7だった。0.55もFollow-throughへscore 1/7のため、M5も再現専用とする。

LightGBMの再現には `--model-type lightgbm` を使う。leaf-wise GBDT、31 leaves、300 trees、learning rate 0.03、min child 100、row/column sample 0.8、L2 5を固定し、後続Platt校正を共通にする。共有計算機では `--lightgbm-n-jobs 8` のように並列数も明示する（既定値 `-1` は全CPU）。M15方向と0.525 confidenceは既存候補に負けるため再現専用。一方、同じ仕様を加工済みbaseline特徴へ適用したM1 standaloneはconfirmation +663件・p=0.0185、全体+938件・p=0.0395で、日次bootstrapも両期間のaccuracy改善を支持した。Pathとは全体4件差、年別4/7、直接差区間0跨ぎのため `config/m1_lightgbm_direction_challenger_v1.json` の異種学習器accuracy co-challengerに固定する。通常25% blendはaccuracy/Brier/log loss 7/7fold改善したが単体より点精度が低く既存proper-score候補も超えないため追加configを発行しない。M1 confidence 0.515はTCNよりaccuracyがbootstrapで明確に低いため使わない。M30への同仕様固定移植では単体がbaseline比+64件、通常25% blendが+37件となった。通常blendと既存Pressure + Ordinal Motif方向候補の固定50/50平均はbaseline比development +16件、confirmation +37件、all +53件、parent比5/7fold・+16件で、baseline Brier/log loss差の日次区間も改善側だった。`config/m30_pressure_ordinal_lightgbm_direction_candidate_v1.json` のparallel co-challengerに固定するがaccuracy差区間は0を跨ぐためauthoritative方向やparent候補を置換しない。M30 confidence 0.515/0.55とPressure・AR・LightGBMの3等分は既存confidence候補を超えず使わない。M5固定移植では単体がbaseline比all +13件でもproper scoreを悪化させ、通常25%方向blendはPressureに負けた。方向維持0.515はProfile×Transitionにaccuracy 0/7、0.55はFollow-throughへ点値5/7でも全期間accuracy/selection scoreの日次区間が0を跨ぎ、confirmationと全行proper scoreが悪化した。上方向×高ボラ0.55も直接bootstrapで未確定、固定50/50 confidence平均も親を超えないため、M5は再現専用とする。
