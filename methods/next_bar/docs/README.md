...
指数減衰する方向・変動状態を加工する場合は `--feature-set ewma_asymmetry_state` を使う。半減期4/16/64のprior-volatility標準化return innovationとdrift/volatility、4対16・16対64のvolatility balance、半減期16/64のupside/downside energy balanceとlagged-return×current-variance leverage momentを固定12列へ圧縮する。全列は[-1,1]、価格scale不変、未来不参照、gapごとにEWMAをreset、flat全0をテストした。M5 Windows canonicalの方向維持25% confidence 0.515はbaseline比accuracy 5/7、selection score 6/7、Brier/log loss 7/7foldを改善し、confirmation accuracy +0.11681ptとscore +0.000681、全期間proper scoreの日次bootstrap区間も改善側だったため `config/m5_ewma_asymmetry_confidence_candidate_v1.json` のparallel broad候補に固定する。Profileとはall accuracy/score差が未確定、固定50/50はconfirmationを下げたため独立維持する。0.55は最新foldが47.60%へ崩れ、down-normal 0.515も局所edge未確認なので、Follow-through 0.55、authoritative confidence、fair odds、売買policyを変更しない。
