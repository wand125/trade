...
指数減衰する方向・変動状態を加工する場合は `--feature-set ewma_asymmetry_state` を使う。半減期4/16/64のprior-volatility標準化return innovationとdrift/volatility、4対16・16対64のvolatility balance、半減期16/64のupside/downside energy balanceとlagged-return×current-variance leverage momentを固定12列へ圧縮する。全列は[-1,1]、価格scale不変、未来不参照、gapごとにEWMAをreset、flat全0をテストした。M5 Windows canonicalの方向維持25% confidence 0.515はbaseline比accuracy 5/7、selection score 6/7、Brier/log loss 7/7foldを改善し、confirmation accuracy +0.11681ptとscore +0.000681、全期間proper scoreの日次bootstrap区間も改善側だったため `config/m5_ewma_asymmetry_confidence_candidate_v1.json` のparallel broad候補に固定する。Profileとはall accuracy/score差が未確定、固定50/50はconfirmationを下げたため独立維持する。0.55は最新foldが47.60%へ崩れ、down-normal 0.515も局所edge未確認なので、Follow-through 0.55、authoritative confidence、fair odds、売買policyを変更しない。同じ定義をM15へ固定移植すると0.515はbaseline/Profileを改善したが、現行Distribution Shiftに全期間accuracy・selection score・Brier・log lossで点劣後し直接bootstrapも全て未確定だった。M30移植も単体方向の点accuracyは上がったが日次区間が0を跨ぎ、方向維持0.52とShiftとの固定50/50平均はShiftにaccuracy・score各1/7foldだった。M15/M30では再現専用とし、それぞれShift 0.515/0.52候補を維持する。
