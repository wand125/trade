...
141. M1固定のRolling Spectral Stateを、64完成足DFT、low/mid/high energy、low-high balance、k1/2/4/8位相の12列、HGB/Platt、25% blend、標準損失1.0のままM5へ移植した。Windows canonical 439,881 OOS行で単体はbaseline比-58件、通常方向blendも-11件、accuracy 4/7のため方向用途を不採用。方向維持0.515はbaseline比accuracy 4/7、score 6/7、proper score 4/7で、confirmation accuracy +0.10288ptだけbootstrap改善側でもcoverage -0.51466ptへ確定低下し、score/proper score区間は0跨ぎ、all主指標も未確定だった。Profile/Haarにはaccuracy/score各3/7対4/7、Profile×Spectral固定平均もProfileに3/7。0.55はFollow-throughよりconfirmation/allのaccuracy・coverage・scoreを下げ、all/confirmation proper scoreもbootstrap劣後側だった。confirmation accuracyは0.515→0.55で52.46888%→58.25359%へ単調上昇し局所整合したが、development/all 0.515は過信、down-normal 4,198件はWilson edge未確認、0.55はup-high 614件以外が疎い。M5 Spectralを再現専用とし、config・registry・authoritative予測・fair odds・policyを変更しない。同履歴でwindow・frequency・band・phase subset・weight・閾値・subgroup filterを再探索せず、共有高負荷処理を停止せず単独8 thread・nice/I/O低優先度・CPU onlyで実行した。

142. M15/M30固定のRolling Ordinal Motifを、3 return・6順位pattern、辞書順tie、32/128本、18特徴、HGB/Platt、25% blend、標準損失1.0のままM5へ移植した。M5の完全18列、56特徴、定常性、価格10倍scale不変、未来側M1改変不影響を追加テストし、Windows canonical 439,881 OOS行を整列した。単体はbaseline比-128件、通常方向blendはdevelopment -10件、confirmation +8件、all -2件、accuracy 4/7、p=0.9899で方向用途を不採用。方向維持0.515はbaseline比confirmation accuracy +0.11162pt、selection score +0.0006552の日次区間が改善側だったがcoverage -0.14519pt、all主指標は未確定、accuracy/score/proper score各4/7foldだった。Profile比all accuracyはほぼ同値でもcoverage -0.15163pt、Brier/log loss悪化区間が確定し、accuracy/score各3/7。EWMAにaccuracy 2/7、Haarに3/7、Profile×TCNに0/7で、Profile×Motif固定平均も親に2/7だった。confirmation accuracyは0.515→0.55で52.47762%→58.50575%へ単調上昇し局所整合したが、0.55はFollow-throughへaccuracy 2/7・score 1/7、test2026_partialは196件・48.97959%、down-normal 4,336件はWilson edge未確認だった。M5 Motifを再現専用とし、config・registry・authoritative予測・fair odds・policyを変更しない。同履歴でmotif長・window・tie・subset・weight・閾値・subgroup filterを再探索せず、共有高負荷処理を停止せず単独8 thread・nice/I/O低優先度・CPU onlyで実行した。

143. M15/M30固定のRolling Autoregressive Stateを、AR(3)、32/128本、scale-adaptive ridge 0.05、15特徴、HGB/Platt、25% blend、標準損失1.0のままM5へ移植した。M5の完全15列、53特徴、定常性、価格10倍scale不変、未来側M1改変不影響を追加テストし、Windows canonical 439,881 OOS行を整列した。単体はbaseline比-282件、通常方向blendはdevelopment +71件、confirmation -87件、all -16件、accuracy 4/7、p=0.8583で方向用途を不採用。方向維持0.515はbaseline比confirmation accuracy +0.11615pt、selection score +0.0005965、all Brier/log lossの日次区間が改善側だったがcoverage -0.57014pt、development objectiveを更新せずall accuracy/score区間も0跨ぎだった。Profile比all accuracy/scoreを下げ、coverage -0.19142pt、Brier/log loss悪化区間が確定し、accuracy 2/7・score 3/7。EWMAに各2/7、Haarに各3/7、Profile×TCNに各0/7で、Profile×AR固定平均もconfirmation scoreを上げずProfile×TCNに各2/7だった。confirmation accuracyは0.515→0.55で52.48215%→57.66423%へ単調上昇し局所整合したが、0.55はFollow-throughへaccuracy 3/7・score 2/7、test2026_partialは202件・48.51485%、down-normal 4,184件はWilson edge未確認だった。M5 ARを再現専用とし、config・registry・authoritative予測・fair odds・policyを変更しない。同履歴でAR次数・window・ridge・subset・weight・閾値・subgroup filterを再探索せず、共有高負荷処理を停止せず単独8 thread・nice/I/O低優先度・CPU onlyで実行した。
144. M1固定のShock / Recovery Stateを、64本reference、2σ、16本追跡、response cap 3、12特徴、HGB/Platt、25% blend、標準損失1.0のままM5へ移植した。完全12列、50特徴、価格10倍scale不変、未来側M1改変不影響を追加テストし、Windows canonical 439,881 OOS行を整列した。通常方向blendはbaseline比development +92件、confirmation +37件、all +129件、accuracy 4/7、p=0.10394、proper score日次区間は改善したが、Pressure比all +33件のaccuracy区間は0跨ぎでBrier/log lossは有意に悪化した。方向維持0.515はbaseline比confirmation accuracy +0.11629pt、score +0.0006260の日次区間を改善したが、all accuracy/scoreは僅かに0跨ぎ、coverage -0.27985ptが確定した。Profile比all accuracy/scoreは僅かに高い点値でもcoverage -0.25030pt、Brier/log loss悪化が確定し、EWMA/Haar/Profile×TCNのaggregate objectiveを更新しなかった。Profileとの固定平均もconfirmation/allを下げた。confirmation accuracyは0.515→0.55で52.48229%→57.96610%へ単調上昇したが、0.55はFollow-throughへaccuracy 2/7・score 1/7、test2026_partial 213件・48.35681%、down-normal 4,198件はWilson edge未確認だった。M5 Shockを再現専用とし、config・registry・authoritative予測・fair odds・policyを変更しない。同履歴でreference・shock閾値・追跡長・response cap・subset・weight・閾値・subgroup filterを再探索せず、共有高負荷処理を停止せず単独8 thread・nice/I/O低優先度・CPU onlyで実行した。
...
136. M5 Rolling Spectral State固定移植は64完成足DFT、固定band/phase、12特徴、HGB/Platt、expanding、uniform sample、標準損失1.0を固定し、window・frequency・band・phase subset・parameter・weight・閾値・subgroup filterを履歴内再探索しない。単体/通常25%方向、方向維持0.515/0.55、Profileとの固定50/50 confidence平均を再現専用とする。confirmationの信頼度単調性は加工感度として保存するが、baseline adoption gateと既存Profile/Haar/Follow-throughの各roleを超えないため、config・registry・authoritative予測・fair odds・policyを変更しない。
137. M5 Rolling Ordinal Motif固定移植は3 return・6順位pattern、辞書順tie、32/128本、18特徴、HGB/Platt、expanding、uniform sample、標準損失1.0を固定し、motif長・window・tie・feature subset・parameter・weight・閾値・subgroup filterを履歴内再探索しない。単体/通常25%方向、方向維持0.515/0.55、Profileとの固定50/50 confidence平均を再現専用とする。confirmationのbaseline改善と信頼度単調性は加工感度として保存するが、Profile proper score、Profile×TCN broad confidence、Follow-through high confidenceを超えないため、config・registry・authoritative予測・fair odds・policyを変更しない。
138. M5 Rolling Autoregressive State固定移植はAR(3)、32/128本、scale-adaptive ridge 0.05、15特徴、HGB/Platt、expanding、uniform sample、標準損失1.0を固定し、AR次数・window・ridge・feature subset・parameter・weight・閾値・subgroup filterを履歴内再探索しない。単体/通常25%方向、方向維持0.515/0.55、Profileとの固定50/50 confidence平均を再現専用とする。confirmationのbaseline改善、all proper score改善、信頼度単調性は加工感度として保存するが、Profile proper score、Profile×TCN broad confidence、Follow-through high confidenceを超えないため、config・registry・authoritative予測・fair odds・policyを変更しない。
139. M5 Shock / Recovery State固定移植は64本reference、2σ、16本追跡、response cap 3、return/rangeの12特徴、HGB/Platt、expanding、uniform sample、標準損失1.0を固定し、reference・shock閾値・追跡長・response cap・feature subset・parameter・weight・閾値・subgroup filterを履歴内再探索しない。単体/通常25%方向、方向維持0.515/0.55、Pressure方向・Profile confidenceとの固定50/50平均を再現専用とする。confirmationのbaseline改善、all proper score改善、信頼度単調性は加工感度として保存するが、Pressure方向、Profile/EWMA/Haar/Profile×TCN broad confidence、Follow-through high confidenceを超えないため、config・registry・authoritative予測・fair odds・policyを変更しない。
