...
- `--execute` を明示した時だけ、許可済みの `methods/swing_eval/analysis/mt5_back_forward_run.py --collect-only` または `methods/swing_eval/analysis/mt5_tester_run.py --collect-only` コマンドを順番に実行する。`--execute` を含むcollect commandや許可外scriptはinvalidとして拒否する。既定のsource runner再評価が失敗した場合は `blocked_queue_refresh_failed` としてcollect実行を止め、保存済みqueueを意図的にそのまま使う時だけ `--no-refresh-queue` を明示する
...
- ワークスペース側 `methods/swing_eval/mt5/TesterSets/*.set` とMT5側 `MQL5/Profiles/Tester/*.set` のhash一致を確認する
- ワークスペース側 `methods/swing_eval/mt5/TesterConfigs/*.ini` とMT5側 `MQL5/Profiles/Tester/*.ini` のhash一致を確認する
- 各 `methods/swing_eval/mt5/TesterConfigs/*.ini` の `ExpertParameters` が参照する `.set` の存在とMT5側同期状態を `tester_config_references` として表示する。score-weight refit用 `.set` はwalk-forward合格後に生成されるため、生成前は `generated_set_missing` として表示し、通常の必須 `.set` 欠落とは区別する
...
`methods/swing_eval/mt5/TesterConfigs/*.ini`:
...
```bash
python3 methods/swing_eval/analysis/dry_run_audit.py \
  --signal runtime/latest_signal.json \
...
```bash
python3 methods/swing_eval/analysis/forward_test.py record \
  --signal runtime/latest_signal.json \
...
```bash
python3 methods/swing_eval/analysis/forward_test.py evaluate \
  --ledger runtime/forward_tests.jsonl \
  --history runtime/latest_history_168h.json \
  --max-hold-minutes 60 \
  --summary-json runtime/latest_forward_test.json \
...
`all_sources_synced=false` はMT5側への `.mq5` 配置漏れ、`all_compiled_fresh=false` はMetaEditorでのCompile漏れ、`all_tester_sets_synced=false` はMT5側 `MQL5/Profiles/Tester` の `.set` 配置漏れまたは古い配置、`all_tester_configs_synced=false` はMT5側 `MQL5/Profiles/Tester` の `.ini` 配置漏れまたは古い配置として扱う。`all_required_tester_config_references_ready=false` は、`.ini` 自体は同期済みでも、その `ExpertParameters` が参照する必須 `.set` が欠落または未同期であることを示す。score-weight refit用 `.set` の生成待ちは `generated_set_missing` として表示し、この必須参照ready判定では許容する。`Swing_Evaluation_Trader.mq5`、`Swing_Evaluation_Predictor.mq5`、`methods/swing_eval/mt5/TesterSets/*.set`、または `methods/swing_eval/mt5/TesterConfigs/*.ini` を編集した後は、Strategy Tester/Forward Test前にこの確認を行う。
...
python3 methods/swing_eval/analysis/mt5_tester_run.py \
  ...
  --template-set methods/swing_eval/mt5/TesterSets/Swing_Evaluation_Trader_buy_entry_refit.set \
...
python3 methods/swing_eval/analysis/mt5_tester_run.py \
  ...
  --template-set methods/swing_eval/mt5/TesterSets/Swing_Evaluation_Trader_buy_hour03_validation.set \
...
```bash
python3 methods/swing_eval/analysis/mt5_tester_run.py \
  --config methods/swing_eval/mt5/TesterConfigs/Swing_Evaluation_Trader_buy_strong_hours_validation.ini \
  ...
  --template-set methods/swing_eval/mt5/TesterSets/Swing_Evaluation_Trader_buy_strong_hours_validation.set \
...
```bash
python3 methods/swing_eval/analysis/mt5_tester_run.py \
  --config methods/swing_eval/mt5/TesterConfigs/Swing_Evaluation_Trader_buy_strong_hours_m30m15_validation.ini \
  ...
  --template-set methods/swing_eval/mt5/TesterSets/Swing_Evaluation_Trader_buy_strong_hours_m30m15_validation.set \
...
python3 methods/swing_eval/analysis/mt5_tester_run.py \
  ...
  --template-set methods/swing_eval/mt5/TesterSets/Swing_Evaluation_Trader_buy_wide_stop_validation.set \
...
python3 methods/swing_eval/analysis/mt5_tester_run.py \
  ...
  --template-set methods/swing_eval/mt5/TesterSets/Swing_Evaluation_Trader_buy_hour03_wide_stop_validation.set \
...
python3 methods/swing_eval/analysis/mt5_tester_run.py \
  ...
  --template-set methods/swing_eval/mt5/TesterSets/Swing_Evaluation_Trader_sell_entry_refit.set \
...
python3 methods/swing_eval/analysis/mt5_tester_run.py \
  ...
  --template-set methods/swing_eval/mt5/TesterSets/Swing_Evaluation_Trader_sell_regime_entry_refit.set \
...
1. `methods/swing_eval/analysis/swing_points.py`
2. `methods/swing_eval/analysis/candidate_generator.py`
3. `methods/swing_eval/analysis/scoring.py`
4. `methods/swing_eval/analysis/backtest.py`
