...
- `src/bridge/mt5_ai_bridge.py` - local HTTP bridge using only Python standard library.
- `src/bridge/request_history.py` - one-time request for a larger historical snapshot.
- `src/bridge/create_trade_command.py` - create a dry-run or live trade command for the EA to validate/execute.
- `src/bridge/sample_request.json` - test payload.
- `src/bridge/config.env.example` - environment variables.
- `methods/swing_eval/mt5/Experts/AI_Bridge_Advisor.mq5` - MT5 Expert Advisor.
- `methods/swing_eval/mt5/Experts/Swing_Evaluation_Trader.mq5` - standalone MT5 EA. No bridge, no GPT.
- `methods/swing_eval/mt5/Indicators/Swing_Evaluation_Predictor.mq5` - standalone MT5 indicator overlay for prediction and dry-run order lines.
...
```bash
python3 src/bridge/mt5_ai_bridge.py
```
...
- Copy `methods/swing_eval/mt5/Experts/AI_Bridge_Advisor.mq5` into your MT5 `MQL5/Experts` folder.
...
- chart-only indicator: `methods/swing_eval/mt5/Indicators/Swing_Evaluation_Predictor.mq5`
- install guide: `docs/mt5-installation-guide.md`
- report: `docs/standalone-mt5-swing-evaluation-trader-report.md`

Install:

- Copy `methods/swing_eval/mt5/Experts/Swing_Evaluation_Trader.mq5` into MT5 `MQL5/Experts`.
- Compile it in MetaEditor.
...
- Copy `methods/swing_eval/mt5/Indicators/Swing_Evaluation_Predictor.mq5` into MT5 `MQL5/Indicators`.
- Compile it in MetaEditor.
...
```bash
python3 methods/swing_eval/analysis/runtime_watchers.py --only mt5_manual_auto_collect --max-heartbeat-age-seconds 180 --mt5-manual-auto-collect-execute-ready
```
...
`methods/swing_eval/mt5/TesterSets/*.set` file is not the same as the MT5 profile copy.
...
curl -s -X POST http://127.0.0.1:8765/snapshot \
  ...
  --data @src/bridge/sample_request.json
```
...
```bash
python3 src/bridge/request_history.py 24
```
...
```bash
python3 src/bridge/create_trade_command.py buy \
