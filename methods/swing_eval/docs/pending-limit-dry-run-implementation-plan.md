...
- `src/bridge/create_trade_command.py` は `buy/sell/close/close_all` のみを受け付ける。
- `src/bridge/mt5_ai_bridge.py` の `TRADE_COMMAND_ACTIONS` も同じ4種類のみを許可する。
- `methods/swing_eval/mt5/Experts/AI_Bridge_Advisor.mq5` は `buy/sell` を `Trade.Buy` / `Trade.Sell` の成行として処理する。
...
- `src/bridge/create_limit_candidate.py`
...
```bash
python3 src/bridge/create_limit_candidate.py buy_limit \
  --symbol XAUUSD-m \
  --entry-price 4127.8 \
