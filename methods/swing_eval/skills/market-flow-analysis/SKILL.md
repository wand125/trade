...
2. If the user asks for historical data that is not saved yet, run `python3 src/bridge/request_history.py 24`, wait for the next MT5 EA post, then read `runtime/latest_history_24h.json`.
3. If `latest_snapshot.json` exists, analyze the raw bars instead of relying only on the markdown summary.
4. If runtime files are missing or stale, say that MT5 data is not available and ask the user to start `python3 src/bridge/mt5_ai_bridge.py` and attach the EA.
