...
def execution_class(execution: dict[str, Any]) -> str:
    ...
    if "methods/swing_eval/analysis/mt5_tester_run.py" in haystack:
        return "mt5_tester_run"
    if "methods/swing_eval/analysis/mt5_compile.py" in haystack:
        return "mt5_compile"
    if "methods/swing_eval/analysis/mt5_agent_csv_archive.py" in haystack:
        return "mt5_agent_csv_archive"
    if "methods/swing_eval/analysis/mt5_forward_collect.py" in haystack:
        return "mt5_forward_collect"
    if "methods/swing_eval/analysis/mt5_optimization_recommend.py" in haystack:
...
def target_matches(target: str, *, key: str, label: str, execution: dict[str, Any], action: dict[str, Any]) -> bool:
    ...
    if normalized in AUTO_MT5_TARGETS:
        return "methods/swing_eval/analysis/mt5_tester_run.py" in command_haystack
...
def execute_plan(
    ...
    *,
    ...
    if execute and plan.get("primary_is_mt5_tester_run") is not True and not allow_non_tester_primary:
        ...
        report["reason"] = (
            "selected primary command does not launch methods/swing_eval/analysis/mt5_tester_run.py; "
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--allow-non-tester-primary",
        action="store_true",
        help="Allow --execute when the selected primary command is not methods/swing_eval/analysis/mt5_tester_run.py.",
    )
