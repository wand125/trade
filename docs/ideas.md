...
- context/session alertとshort losing monthのANDは00191と同じ成績で、global month-level budget0 triggerの上積みにはならなかった。次は alert contextだけに `context_entry_budget=0/1` または追加entry marginを掛ける、context-specific first-loss capへ進む。
