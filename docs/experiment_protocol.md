...
`docs/reports/` に置くレポートのファイル名は `00001_YYYY-MM-DD_slug.md` の通し番号形式にする。通し番号はファイルシステムの更新時刻や `更新日時` ではなく、レポートファイル内の `日時: YYYY-MM-DD HH:MM JST` の昇順で決める。既存レポートを確認・再採番する場合も、mtimeではなくファイル内の `日時` を参照する。直近・最新レポートを判断する場合もファイル内の `日時` を正とし、通し番号はその順序に由来する補助情報として扱う。新規レポートは既存レポートの最大番号の次を使う。
