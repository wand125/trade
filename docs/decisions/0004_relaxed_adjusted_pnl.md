...
- その後、教師生成とvalidation/test評価のズレを診断するため、profit 1.0 / loss 1.2 に揃えたdatasetを作成し、80/320/1280/低LR1280を比較した。
- 1.0/1.2 aligned学習はvalidation上の候補を作ったが、fixed testではNoTradeを超えなかった。
- そのため、1.0/1.2 aligned学習は現時点では診断実験として扱い、標準フローへの昇格は保留する。
- 以後の実験では、`train target倍率` と `validation/test評価倍率` を必ず明記する。
