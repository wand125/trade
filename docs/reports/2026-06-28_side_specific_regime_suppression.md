...
1. 2025-04以降のblind holdoutを追加し、`short:session_regime=asia` を事前登録候補として検証する。
2. `model-candidate-selection` にside/regime別損失集中を追加し、今回のような最大損失依存をvalidation内で検知できるようにする。
3. exit timing targetを追加し、block後にも残る `exit_regret_sum=702.5012` を減らす。
