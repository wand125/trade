...
1. Horizon-specific binary viability / abstention-first decisionを作る。`fixed60_executable`, `fixed240_executable`, `fixed720_executable` を別々に予測し、calibrated probabilityとexpected PnLが両方通る時だけhorizonを選ぶ。
2. 720m tail-loss classifierを追加し、long-horizon exitを開く前に悪い720m short tailを止める。
3. one-fail strict rowsを教師候補/診断候補として残すが、entryへ直接昇格しない。
4. Side-balanced support overlayは、horizon-specific viability headを通した後にstateful replayする。
4. repair targetを候補比較の補助ゲートにする。totalが改善してもrepair targetが不変なら、標準化への前進とは扱わない。
5. Context-count shrinkageではなく、OOF calibrated EV uncertaintyをcontinuous featureとしてexpected PnLへ入れる。
6. entry-block候補は今後もholdout-support diagnosticsを通し、発火0件や1件支持ならhard blockへ昇格しない。
7. `large_loss_uncompensated_by_context` は教師候補として残し、entry/exit sequence features、replacement state、skipped next winner costと組み合わせる。
8. sequence-state診断の `next_*` はerror analysis専用にし、実行時featureは `prev_*`, 月内trade count, prior-only contextに限定する。
9. `isolated_large_loss_long + fixed720 + threshold -5 + replacement + require-model-used` はdiagnostic branchとして維持し、extension veto悪化とentry-time proxyのrefit集中も含めて標準policyにはしない。
