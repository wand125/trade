...
| Latest support surface | `00382` | all-family priorとshrunk priorをselector surfaceへ接続。candidate gapは非oracle/ oracleとも `3 -> 2` に減ったが、target outcome passは0/48。support増加は前進だがcalibration安全性は未解決で、direct policyはreject。 |
...
2. support-sufficient negative month laneは、current-branch negative repairとcross-artifact robustnessを分ける。00379でcross-family priorはsupportを作るが不安定、00380/00381で制約通過はtarget修復成功と同義ではなく、00382でshrunk cross-family priorでもtarget outcome pass 0と分かったため、support-limited candidate generationとsupport-sufficient replacement calibrationを別診断にする。
