...
1. Horizon-specific viability headはdirect selectorではなくfeature/diagnosticとして維持し、より広いcandidate universeで再学習する。
2. 720m tail-loss classifierとPnL regressorのcalibrationを優先し、hybrid 2025-11 shortのような悪い720m pathを止められるまでsupport overlayへ接続しない。
3. one-fail strict rowsを教師候補/診断候補として残すが、entryへ直接昇格しない。
4. Side-balanced support overlayは、calibrated viability featureを通した後にstateful replayする。
