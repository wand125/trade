...
1. Entry admission reviewは `--multi-window` selectorを標準入口にする。単一2ヶ月validationだけで標準候補を選ばない。
...
4. raw EV絶対値ではなく、side/regime別のrank、calibrated EV quantile、support-aware thresholdをadmission特徴として評価し、複数validation windowでtotal、worst、trade support、side balance、regime worst bucketがNoTradeを超えるかを見る。
