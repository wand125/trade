...
1. `large_loss_uncompensated_by_context` / negative path contextを教師候補にする。ただし同月実現context PnLは未来情報なので実行時featureには使わない。
2. large-loss probabilityはhard gateではなく、candidate-level selector / stateful replay / exit timing targetの補助featureへ回す。
3. `isolated_large_loss_long + fixed720 + threshold -5 + residual combo block` はdiagnostic branchとして維持し、標準policyにはしない。
4. support-aware分類を今後の候補比較に使うが、通過だけで標準化しない。
5. `support_aware_only` 候補は標準policyにせず、unused chronologyまたはmodel-level calibration reasonが出るまでNoTradeを維持する。
6. thin-support residual monthsはbroad month-warmupやconfidence hard gateではなく、unused chronologyまたはchronological calibration側で確認する。
7. calibration scoreはgateではなくuncertainty / regime diagnostics / admission explanationへ使う。
8. `long|range_normal_vol|ny_overlap` と `short|down_normal_vol|asia` は次のresidual-risk targetの監査対象にする。
9. fixed-horizon rescueを試す場合は、hindsight deltaではなくchronological prediction/selector replayへ戻す。
