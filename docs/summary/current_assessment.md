...
| Entry EV executable EV stateful score | prediction全行にprior-only executable capture factorを付け、`timed_ev` のlong/short scoreを置換 | refit2025 long shareは `0.9169..0.9150 -> 0.4367..0.4705`。`720m q99 floor5` は validation total `+43.0418`, min role `+2.4158`, min month `-1.8000`, trades `19`。floorなしは q99 validation `-51.2934`、floor `2/3/4` もtail悪化 | accepted infrastructure。tested policiesはNoTrade-first gate未達 |
| Entry EV dense executable capture model | prediction全行をside-row化し、fixed720/fixed240 capture factorを月順OOF回帰 | fixed720 MAEは `2025-01 17.0734 -> 10.6154`, `2025-02 19.9742 -> 13.9449`。fixed240 MAEは `2025-01 16.0786 -> 6.0691`, `2025-02 17.4734 -> 8.1887`。ただしfixed720 q95 floor5は validation total `+16.4192` でもfresh role `-76.2788`; fixed240も全候補NoTrade | accepted infrastructure。direct score replacementは標準採用しない |
| Entry EV side-balance score penalty | prior predicted-vs-target side share driftでdense executable scoreの過剰sideを縮小 | refit2025 long shareは `2025-01 0.9453 -> 0.8911`, `2025-02 0.8970 -> 0.8750` に改善し、refit q95 floor5は `+93.9912`。一方 fresh q95 floor5は `-82.2428`, overall q95 floor5は total `+14.6138` でも min role `-82.2428`, min month `-46.5308` | accepted infrastructure。generic direct penaltyは標準採用しない。selector/ranking/downside-conditioned featureへ移す |
| Entry EV side-balance feature diagnostics | side-balance driftをselected-trade featureとして再結合し、post-hoc screenを診断 | fresh q95 floor5は total `-82.2428`, overrep share `0.3750`。refit q95 floor5は total `+93.9912`, overrep share `0.4737`。q99 floor5の `selected_underrepresented>=0.02` は11 trades / `-16.8394` を拾うが、q95 floor5の同系screenは利益を削る | accepted diagnostic。side-balance単独gateは採用しない。prior downside evidenceとの相互作用へ進む |
| Entry EV side-balance downside interaction | side-balance driftとprior downside evidenceを掛け合わせて診断 | q99 floor5は `risk>=0.20` で7 trades / `-31.1784` を拾い kept total `+20.9498` だが、kept min role `-30.9390`, kept min month `-33.4920`。q95 floor5はtailが残り、refit勝ちroleにも高risk/high interactionが出る | accepted diagnostic。hard gate/direct penaltyは採用しない。selector/ranking/downside-weighted targetへ移す |
| Entry EV side-balance downside selector | side-balance/downside pressureをcandidate-level selector特徴へ集約 | strict selectorは全候補NoTrade。relaxedではq99/q95 floor10がeligibleだが、active role 2、prior zero share `0.9000..0.9130`。q95 floor5はpressure `0.3116`, uncovered loss `-153.0528` でfresh tailを示す | accepted diagnostic。pressure単独selectorは採用しない。support/coverage制約と併用する |
| Entry EV side-balance downside coverage audit | required role coverageとprior supportを明示監査 | strict/relaxed coverage gateとも全候補NoTrade。216 sensitivity rowsも全てNoTrade。floor10はfresh欠損、floor5はrole/month tailとcal2024 prior-zeroで落ちる | accepted diagnostic。coverage/support preflightとして使う。現候補は採用しない |
| Entry EV side-balance downside composite selector | coverage/support、side-balance/downside、direction/exit、EV overestimateを同じgateへ集約 | strict/relaxedとも全候補NoTrade。288 sensitivity rowsも全てNoTrade。floor10はfresh欠損で composite risk `1.0`、floor5はfresh tail、prior-zero、direction error、EV過大評価で落ちる | accepted diagnostic。missing roleをunknown/high riskとして扱う。現候補は採用しない |
| Entry EV composite target decomposition | compositeをmodel-time featureとtraining/evaluation targetへ分解 | 115 tradesを分解。`none` overlapは 14 trades / `+176.8770`。direction + exit + EV overestimate + realized loss overlapは 11 trades / `-96.4764`、direction + large exit + EV overestimate + realized lossは 8 trades / `-146.2824` | accepted target frame。単一binary no-trade labelにはしない。target別headへ進む |
| Entry EV component target calibration | component targetをsupport/pressure bucketで低容量calibration診断 | chronological mean AUCはEV overestimate `0.6741`、direction `0.2644`、exit `0.4457`、loss `0.4819`。role holdoutでもEV overestimate `0.6401` だけが相対的に残る | accepted diagnostic。EV overestimateはtarget head候補。support/pressureだけでdirection/exit/lossは判断しない |
| Entry EV overestimate risk selector | EV overestimate target riskを候補selector featureへ戻す | strict/relaxedとも全候補NoTrade。risk sensitivity 480行も全てNoTrade。q95 floor5 pointwise high-risk screenは24 trades / `-35.7612` を拾い kept total `+50.3750` だが、refit勝ちroleにも high-risk `+46.1476` | accepted diagnostic。hard gateではなくranking/calibration headへ移す |
...
- entry EV executable EV stateful score inputs / diagnostics
- entry EV dense executable capture model diagnostics
- entry EV side-balance score input diagnostics
- entry EV side-balance selected-trade feature diagnostics
- entry EV side-balance downside interaction diagnostics
- entry EV side-balance downside selector diagnostics
- entry EV side-balance downside coverage audit diagnostics
- entry EV side-balance downside composite selector diagnostics
- entry EV composite target decomposition diagnostics
- entry EV component target calibration diagnostics
- entry EV overestimate risk selector diagnostics
...
- promoting executable-EV stateful q99 floor5 after 00231, because it still fails validation month floor and trade support
- promoting fixed720/fixed240 dense capture scores after 00232, because row-level MAE improvements do not survive NoTrade-first stateful validation
- promoting generic side-balance direct penalty after 00233, because refit long-share improvement came with fresh validation tail losses
- promoting side-balance-only selected-trade screens after 00234, because q99 post-hoc improvements do not generalize to q95 and do not model replacements
- promoting side-balance x downside hard gates after 00235, because q99 pointwise gains still leave negative role/month floors, q95 tails remain, and high-risk interaction also appears in winning refit contexts
- promoting low side-balance/downside feature pressure candidates after 00236, because the relaxed selector prefers floor10 candidates with weak role coverage and prior-zero evidence
- promoting any current side-balance/downside candidate after 00237, because coverage/support-aware gates still select NoTrade across strict, relaxed, and all sensitivity rows
- promoting any current composite side-balance/downside candidate after 00238, because strict/relaxed gates and 288 sensitivity rows still select NoTrade, and EV overestimate is only a validation calibration diagnostic
- using `composite_failure_target` from 00239 as a single no-trade label, because target-active profitable overlaps exist and component targets have different meanings
- using `support_bucket + pressure_bucket` from 00240 as a sufficient target model for direction/exit/loss, because only EV overestimate keeps useful rank signal across chronological and role-holdout checks
- promoting predicted EV-overestimate risk selector gates from 00241, because NoTrade-first sensitivity still selects NoTrade and pointwise high-risk screens also remove refit winning trades
...
- 00231でentry時点のlong/short scoreそのものをexecutable EVへ差し替えると、refit2025のlong過剰は大きく改善した。ただし最良の `720m q99 floor5` でも validation min month `-1.8000`、0-trade月、fresh fixed `2024-10 -10.3560` が残る。floorなしとfloor `2/3/4` はtailを再導入し、細密floor探索に安定台地はない
- 00232でdense capture targetを学習するとrow-level EV MAEは大きく改善するが、取引成績には転化しない。fixed720はfresh role tailを壊し、fixed240はscoreを縮めすぎてもfresh tailとsupport不足が残る。generic fixed-horizon captureを直接scoreにするだけでは、一玉制約下のadmission品質やside balanceを保証しない
- 00233でprior-only side-balance penaltyをdirect scoreへ掛けると、refit2025のlong過剰は縮むが、fresh2024ではshort側を縮めてlong shareを増やす月があり、fresh validation tailが悪化した。side driftは無条件に補正するのではなく、prior downside evidenceと一緒にselector/ranking featureへ入れる必要がある
- 00234でside-balanceをselected-trade featureとして見ると、q99ではpost-hocに損失を拾うscreenがあるが、q95では同じscreenが利益を削った。fresh最大損失の一部はunderrepresented側またはdrift閾値未満で、side-balance drift単体ではtail説明力が足りない
- 00235でside-balance x prior downside interactionを見ると、q99 floor5の一部損失は拾えるが、fresh最大損失はprior support 0またはlow driftで残り、q95ではrole tailが残る。さらにrefit勝ちroleにも高risk/high interactionがあるため、interactionはhard gateではなく低容量featureとして扱う
- 00236でcandidate-levelに集約すると、pressureはfloor5 fresh tailを示すが、低pressure候補はfloor10系でfresh role coverageとprior evidenceが薄い。したがってpressure scoreはsupport/coverage制約なしでselectorに使えない
- 00237でrequired role coverageを明示すると、floor10はfresh欠損、floor5はfresh tailとcal2024 prior-zeroで落ちる。coverage/support-aware sensitivityでも全てNoTradeなので、現candidate群から採用候補を選ぶのではなく、missing-support indicatorを学習特徴へ入れる
- 00238でcoverage/supportにdirection/exit failureとEV overestimateを足しても、strict/relaxed/sensitivityは全てNoTradeだった。missing roleを低risk扱いしない方針は正しいが、composite hard gateを増やすだけでは候補は生まれない。次はcompositeを学習target/featureへ分解する
- 00239でcompositeをtrade単位のfeature/targetへ分解した。target-activeでも利益になるoverlapがあるため、long/short/stay-flatや単一no-trade labelに圧縮せず、direction-side inversion、exit capture、EV overestimate、realized lossを別target headとして扱う
- 00240でsupport/pressure bucketの低容量calibrationを試すと、EV overestimateだけがchronological / role holdoutで残り、direction/exit/lossは弱いか逆向きだった。したがって次はEV overestimateをcalibration headにし、direction/exitはside/context/holding/capture特徴を足して別head化する
- 00241でEV overestimate riskをselector featureへ戻すと、fresh q95/q99 floor5損失は説明できるが、同じhigh-risk featureがrefit勝ちroleにも強く出る。したがってEV overestimateは削除gateではなく、side/context付きrankingやcalibration headとして使う

したがって、次の改善は「holding capの再探索」でも「floor閾値の細密探索」でもなく、entry EV calibration、rank/quantile admission control、prior-only context-side inversion、exit captureを、より多いchronological validation window / purged walk-forward / regime別安定性評価へ分解して進めることを優先する。dense/executable EVは採用候補ではなく、side balance、downside-weighted target、selector/ranking featureとして強化する。
...
5. 00226..00241の prior risk / executable EV系はインフラとして残すが、現blocking rule、局所threshold低下、q99 sparse near-miss、fixed-horizon dense direct score、generic side-balance direct penalty、side-balance-only selected-trade screen、side-balance x downside hard gate、low-pressure-only selector、coverage/support-failing candidates、composite hard gate candidates、composite単一binary label、support/pressureだけのtarget model、EV-overestimate risk hard selectorは標準採用しない。次はprior rolesを事前固定した複数chronological windowで、EV overestimateをcalibration head、direction/exitをside/context/holding/capture特徴付きhead、risk scoreやcapture-calibrated EVをhard blockではなくdownside-weighted dense target、rank特徴、selector featureとして評価する。
6. side prior driftを、predicted side share vs dense label side share の prior window差分として特徴量化し、prior side PnL / direction error / exit capture failure / context loss と組み合わせたdownside-conditioned correctionにする。
...
- `00227`: entry EV residual 2024-03 loss diagnostics
- `00228`: entry EV exit capture target diagnostics
- `00229`: entry EV executable EV calibration
- `00230`: entry EV executable EV selector feature
- `00231`: entry EV executable EV stateful score
- `00232`: entry EV dense executable capture model
- `00233`: entry EV side-balance score penalty
- `00234`: entry EV side-balance feature diagnostics
- `00235`: entry EV side-balance downside interaction
- `00236`: entry EV side-balance downside selector
- `00237`: entry EV side-balance downside coverage audit
- `00238`: entry EV side-balance downside composite selector
- `00239`: entry EV composite target decomposition
- `00240`: entry EV component target calibration
- `00241`: entry EV overestimate risk selector
