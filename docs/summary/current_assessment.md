...
1. Singleton negative group diagnosticsをabstention layerへ接続し、唯一候補が弱いときにsupport目的だけで入らない設計にする。
