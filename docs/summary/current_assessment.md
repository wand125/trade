...
- 00206で早期2024のrisk OOFを `2024-05` まで前倒しした。純2024 bridgeではsource p10/replm10が合計最良だが、no-side-penaltyの方がworst monthとdrawdownが良く、side hookの標準採用根拠にはならない

したがって、次の改善は「holding capの再探索」でも「2025系列へのshort hook追加」でもなく、side/EV calibration、admission control、そして2024全体を同一chronological protocolで揃えるかの判断を優先する。

## 次に検証すべきこと

1. 2024全体を、今回のbridgeではなく一つのchronological training protocolで再生成するか判断する。混合familyのまま `gap0/gap5/budget0` へ進むと、policy差とprediction-family差が混ざる。
...
4. short/range_low_vol の context drawdownを、現在月のrealized PnLだけで発火させる低容量hookとして評価する。ただし00204/00206の反省から、同じ系列だけで採用判断しない。
