...
5. 00226..00234の prior risk / executable EV系はインフラとして残すが、現blocking rule、局所threshold低下、q99 sparse near-miss、fixed-horizon dense direct score、generic side-balance direct penalty、side-balance-only selected-trade screenは標準採用しない。次はprior rolesを事前固定した複数chronological windowで、risk scoreやcapture-calibrated EVをhard blockではなくdownside-weighted dense target、rank特徴、selector featureとして評価する。
