...
1. support-aware分類を使い、候補比較でfloor-breach classが安定するかを追加chronologyへ広げる。
2. `support_aware_only` 候補は標準policyにせず、unused chronologyまたはpolicy-level reasonが出るまでNoTradeを維持する。
3. `isolated_large_loss_long + fixed720 + threshold -5 + residual combo block` はdiagnostic branchとして維持し、標準policyにはしない。
4. fixed-horizon rescueを試す場合は、hindsight deltaではなくchronological prediction/selector replayへ戻す。
