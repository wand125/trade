...
| Current diagnostic candidate | q95 + `loss_exit30` dynamic exit。pre-standard diagnostic candidateへ昇格。`direction_regime` tail-riskはdiagnostic featureへ降格 |
| Why not standard | `00277` 統合では positive roles `6/6` だが month min `-11.3450` が残る。trade delta上、追加entry netはプラスだが added negative `-123.0768` が大きい |
...
| Component / exit-regret | `00240`..`00277` | EV overestimateからdirection/exit/replacementへ分解。00267でq99 prior guardがstateful replay上は改善したが、標準admission未通過。00268でfresh support不足がepisode集中であり、rank0緩和はcal/refitを壊すと確認。00269の外部HGB、00270の外部full-hybridでもNoTrade未満。00271で損失はno-edgeではなくexit-capture failure / executable EV過大評価に寄ると確認。00272でpost-selector executable scoreは負の対照としてreject。00273でselector前capture補正もNoTrade未満。00274でcoarse `direction_regime` tail-riskはq99をプラス化したが、support/side集中でNoTrade。00275で外部HGB再現は弱く、tail-risk headはdiagnosticへ降格。00276/00277でlow loss-first dynamic exitが強い改善を見せ、全role positiveまで進んだが、month floorと追加entry負けが残る。 |
...
1. variant delta rowsから悪いonly-candidate追加entryを狙い、`loss_exit30` の過剰回転を抑える。
2. absolute loss-first probability thresholdではなく、対象月以前のvalidation分布に基づくquantile / calibrated thresholdへ置き換える。
3. dynamic exit後のminimum hold / cooldownを試し、May/June tail改善を保ったまま追加entry損失を減らせるか確認する。
