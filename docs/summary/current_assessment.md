...
1. Thin monthsのprediction rowsからnear-miss opposite-side candidatesを探し、同じ一玉制約下で追加/置換できるかをstatefulに検証する。
2. Side-balanced support overlayを設計し、month support / side-shareを改善してもNoTrade-first gatesとrole/month floorを壊さないか見る。
3. repair targetを候補比較の補助ゲートにする。totalが改善してもrepair targetが不変なら、標準化への前進とは扱わない。
4. Context-count shrinkageではなく、OOF calibrated EV uncertaintyをcontinuous featureとしてexpected PnLへ入れる。
5. entry-block候補は今後もholdout-support diagnosticsを通し、発火0件や1件支持ならhard blockへ昇格しない。
6. Short replacementは全family一律ではなく、family / regime / prior context supportで分割して再評価する。
