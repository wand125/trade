...
1. Near-miss pool用のchronological exit-viability / horizon headを作る。`target_fixed_executable`、`target_fixed_best_horizon_minutes`、calibrated selected-horizon EVを予測する。
2. one-fail strict rowsを教師候補/診断候補として残すが、entryへ直接昇格しない。
3. Side-balanced support overlayは、chronological exit headを通した後にstateful replayする。
