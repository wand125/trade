...
00376時点では、00370 inventoryに出ている既存artifactをselector surfaceへ流用できるか棚卸しした。17 metric parents中、surface-ready without conversionは0件。10件はtrade schema readyで、`00310_position_quality_proxy_overlay_s1` と `short_entryblock_replacement_holdext_block_overlay_s1` は次のconfig合成候補。ただし17件すべてsurface config key (`current_trades`, `family_predictions`, `candidate`) が不足し、hold-extension stateful系7件は `entry_block_rule`, `entry_blocked`, `selector_variant` のschema変換も必要。次はschema-ready artifactへ00318 configを合成し、`holdext_isolated_large_loss_t-5_h720` と `holdext_isolated_large_loss_long_t-5_h720` のvariant familyを混ぜない形でsurface replayする。

00377時点では、`00310_position_quality_proxy_overlay_s1` へ00318 configを合成し、`loss_exit30_cd15__holdext_isolated_large_loss_long_t-5_h720` 系に揃えたcurrent-negative surfaceを実行した。4 target評価で、非oracle `side_gap` + `prior_actual_mean` はloss selected 3 / winner selected 0 / precision 1.0 / mean delta `+5.0618` で制約通過。ただし改善は `refit2025 2025-03` の `+20.2470` に集中し、`fresh2024 2024-03/11` はsupported candidate 0、`hybrid2025_0912 2025-11` はrisk trade選択なしだった。これはmulti-target repairではなく、support-limited candidate generation不足を示すstress結果。次はsupport-sufficient repair laneとsupport-limited candidate-generation laneを分ける。

00378時点では、`short_entryblock_replacement_holdext_block_overlay_s1` でもaligned surfaceを実行した。5 current-negative targetを評価し、support-sufficient targetは `hgb2024_0306 2024-03` と `refit2025 2025-03` の2件へ増えた。非oracle `ev_ge5` + `prior_actual_mean` はloss selected 4 / winner 0 / precision 1.0で制約通過したが、mean after PnLは `-0.3246` で、改善は `refit2025 2025-03` の `+18.2394` のみ。`hgb2024_0306 2024-03` はloss tradeを選べてもsupported replacement candidate 0で `-17.6936` が残る。主課題はrisk selectorだけでなく、same-family priorがないearly-month replacement calibration不足。

00379時点では、replacement calibration診断へ `prior_scope` と `require_supported_candidates` を追加した。all-family priorは `hgb2024_0306 2024-03` のsupportを prior rows `0 -> 276`、supported candidates `0 -> 1211` に増やせる。ただしcalibrationは不安定で、全体では `side_score` mean month PnL `-5.5427`、`prior_actual_mean` `-13.1994`。hgb worst lossだけなら `side_score` が月PnLを `+10.5674` にできるが、raw/bias/conservative系は悪い720m candidateを選び `-43.5812` へ悪化する。cross-family priorはsupport sourceとして診断採用するが、direct policy採用はreject。

00380時点では、00378 surfaceの制約通過をtarget-level outcomeへ分解した。非oracle `feature:ev_ge5_lossfirst_lt0p30` + `prior_actual_mean` は5 target中、修復成功1、candidate gap 3、risk gap 1で、mean after PnLは `-0.3246` のまま。`combined:any_lossrisk` + `prior_actual_mean` はmean after `+3.5350` だがwinner risk 2を含む。つまり、loss selected / winner selected制約だけでは「直せたtarget」と「候補不足でbaseline維持したtarget」を分離できない。target outcome categoryは採用し、今後のsurface rankingにはsuccess count / candidate gap / risk gapを明示する。

00381時点では、target outcome categoryをselector surface本体へ統合した。`support_sufficient_selector_surface_choices.csv` に outcome columns を出し、summaryには `passes_target_outcome_constraints` を追加した。00378同条件の再実行では、旧winner-damage制約は16行中8行が通過したが、新target outcome制約は16行中0行。oracleでもsuccess 2 / candidate gap 3で落ちるため、risk selectionだけでなくcandidate supportを解かない限り標準policyには進まない。

00382時点では、all-family priorと `shrunk_prior_actual_mean` をselector surfaceへ接続した。00378同条件で48 rowsを評価し、旧winner-damage制約は20行が通過したが、target outcome制約は0行。all-family priorは非oracle `ev_ge5` のcandidate gapを `3 -> 2`、oracleのcandidate gapを `3 -> 2` に減らしたが、shrunk priorはraw priorと同じ選択になり、outcome passには届かなかった。`side_score` はmean PnLを大きく改善するがcalibrated EVではないため、直接policy evidenceとして扱わない。次はsupport-limited candidate generationとsupport-sufficient replacement calibrationを分ける。

00383時点では、00382のcandidate gapをsupport filter別に分解した。残gapは `prior_count_gap` ではなく `prior_month_and_actual_gap`。非oracle `ev_ge5` + `shrunk_prior_actual_mean` では、`hgb2024_0306 2024-03` と `fresh2024 2024-03` にcandidate poolとpositive actual candidateがあるが、prior countは通ってもprior month passは0、max prior month countは1、max prior actual meanは `-1.3401`。oracleでも同じ2targetが残る。したがってearly-month targetはraw candidate生成不足ではなく、薄くて負のprior候補をどの条件なら信用できるかの問題。次はprior_month_count=1候補をpolicy replayではなくsupport-limited scoring reportで診断する。
...
00367..00381追加リスク: replacement改善をloss trade事前識別の証拠と誤読しない。min prior month `1` の高PnL候補をsupport十分と扱わない。winnerを外してもreplacementで月PnLが改善したケースをloss-risk selector成功と扱わない。one-fail replacement候補の単一target月改善を標準policyへ昇格しない。auto target count 1を複数target検証済みと誤読しない。00370のconfig rows / variant rowsを独立サンプル数として扱わない。realized monthly PnLで作ったtarget inventoryを実行時featureやpolicy evidenceとして扱わない。00371のcross-artifact target identityでbaselineがpositiveの月を、current negative-month repair成功と誤読しない。winnerを外してreplacementで上回ったケースをloss-risk selectorの精度として扱わない。00372/00373でoracle loss selectionでもbaseline-positive degradationが残るため、risk selectorだけを改善すれば十分と扱わない。winner-damage rankingで上位でも制約未通過ならpolicy候補と扱わない。00374/00375のabstention通過を、target 1件改善の薄さやpost-surface閾値選択を無視して標準policy化しない。広いtarget setで壊れなかったことを複数target有効性と誤読しない。00376のreadinessでtrade schema readyなartifactを、config/variant整合抜きでsurface evidenceとして扱わない。00377/00378のsupport-limited or early-month no-candidate baseline維持をrepair成功と扱わない。00379のcross-family prior support増加をcalibration安全性と誤読しない。00380のloss selected countやconstraint passをrepair success countとして読まない。00381のold winner-damage passをtarget outcome passと混同しない。

00382追加リスク: all-family priorでcandidate gapが減ったことを、replacement calibrationの成功と誤読しない。`shrunk_prior_actual_mean` がraw priorと同じ選択になる場合、shrinkage導入だけでは過大評価補正済みとは扱わない。`side_score` のmean PnL改善をcalibrated expected PnLとして標準policy化しない。target outcome pass 0のsurfaceを、旧winner-damage pass数やmean PnLだけで採用しない。

00383追加リスク: candidate poolやpositive actual candidateが存在することを、実行時support十分と誤読しない。prior month countが1でprior actual meanが負の候補を、oracle actualだけで採用しない。`prior_month_count=1` への緩和はpolicy replayではなく、追加confidence signalの診断として扱う。
...
2. support-sufficient negative month laneは、current-branch negative repairとcross-artifact robustnessを分ける。00379でcross-family priorはsupportを作るが不安定、00380/00381で制約通過はtarget修復成功と同義ではなく、00382でshrunk cross-family priorでもtarget outcome pass 0、00383で残gapはprior month 1 + negative prior meanと分かったため、support-limited candidate scoring reportとsupport-sufficient replacement calibrationを別診断にする。
...
7. 00329/00335 leak-free bestのremaining weak months、特に `fresh2024 2024-03`, `fresh2024 2024-08`, `fresh2024 2024-11`, `refit2025 2025-03` を00331..00383 target labelsで診断する。単純fallback緩和、global early-support relaxation、train-count gate、reliability direct multiplier、stateful pred-pnl negative veto、positive-PnL hard gate/soft penalty、global harmful/residual/720m gate、薄いexact-context hard gate、coarse diagnosticだけのpositive-bias hard prefilter/soft penalty、tail-pass residual global hard gate、unique 1件支持のselected pred_pnl_lt2標準化、support-sufficient negative monthのthin-support混入、predicted fixed-horizon argmax直用、単一target月のone-fail replacement標準化、variant inventory countの独立サンプル化、baseline-positive targetのmean改善による標準化、oracleでも悪化するreplacement selectorの標準化、winner-damage ranking上位の制約未通過採用、target 1件だけのabstention gate標準化、広いtarget setで壊れなかっただけのgate標準化、schema/config未整合artifactのsurface evidence化、support-limited or early-month no-candidate baseline維持のrepair成功扱い、cross-family prior support増加だけでのpolicy採用、loss selected countをrepair success countとして読むこと、old winner-damage passをoutcome passと混同すること、`side_score` のmean PnL改善をcalibrated EVとして扱うこと、target outcome pass 0のshrunk prior surfaceを採用すること、prior month 1 + negative prior meanのcandidateをpositive actualだけで採用することは本流にしない。
