...
1. 別family / 別期間 / 追加support-repair surfaceでsingleton事例を増やし、00338の`singleton_720_pred_pnl_lt2`をpre-registered diagnostic guardとして再検証する。
2. fresh/thin month用の候補生成を作る。singletonをabstainした後にsupport/side blockerを悪化させない代替候補を探す。
3. harmful probabilityをhorizon / side / session / regime / support bucket別にcalibrateし、global scalar penaltyや単純なharmful-lower switch ruleとして使わない。
4. 00329/00335 leak-free bestのremaining weak months、特に `fresh2024 2024-03`, `fresh2024 2024-08`, `fresh2024 2024-11`, `refit2025 2025-03` を00331..00336 target labelsで診断する。単純fallback緩和は本流にしない。
5. `role_trades_low` と `side_share_high` を解く候補は、non-negative floor contributionを必須にし、EV-2の負け候補をsupport目的だけで入れない。
6. thin month / opposite-side supportを目的関数に明示し、単純なhorizon EV最大化だけでrepair targetが不変になる状態を避ける。
7. actual-floorは診断専用に残し、実行可能policyではobservable / learned proxyで代替する。
8. fallback0.25とfallback-allowed rankerはdiagnostic sensitivityとして維持し、learned evidenceや標準policy値として扱わない。
9. repair targetを候補比較の補助ゲートにする。totalが改善してもrepair targetが不変なら、標準化への前進とは扱わない。
