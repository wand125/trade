...
1. Hold-extension state-dependent `holdext_long_range_normal_ny` を、実行時に使えるproxyまたはfull replay pathへ戻す。
2. Side-aware fixed 720m hold-extensionとshort entry-block replacementを同じstateful replay内で統合する。
...
7. `isolated_large_loss_long + fixed720 + threshold -5 + residual combo block` はdiagnostic branchとして維持し、標準policyにはしない。
