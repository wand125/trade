...
00367..00369追加リスク: replacement改善をloss trade事前識別の証拠と誤読しない。min prior month `1` の高PnL候補をsupport十分と扱わない。winnerを外してもreplacementで月PnLが改善したケースをloss-risk selector成功と扱わない。one-fail replacement候補の単一target月改善を標準policyへ昇格しない。auto target count 1を複数target検証済みと誤読しない。
...
2. support-sufficient negative month laneは、00369で現branchのtarget countが1件と確認済み。他branch/variantでsupport-sufficient negative monthを探すか、support-limited negative month laneを別目的関数として作る。
