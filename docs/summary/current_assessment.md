...
5. 00226/00227の `prior_risk` はインフラとして残すが、現blocking ruleと局所threshold低下は標準採用しない。次はprior rolesを事前固定した複数chronological windowで、risk scoreをhard blockではなくrank特徴やselector featureとして評価する。
