...
1. Near-miss support candidate用のexit timing targetを作る。fixed60/fixed240/fixed720で生きる候補とoracle bestだけ良い候補を分ける。
2. one-fail strict rowsを教師候補/診断候補として残すが、entryへ直接昇格しない。
3. Side-balanced support overlayは、exit timing / EV calibrationを通した後にstateful replayする。
