...
| Support repair continuation | `00323`..`00329` | 00322 s2を00314 best branchのsupport repairへ接続。00323 best totalは5本追加 / `+23.4090` / combined `+362.7000`。00325 actual-floor upper-boundはcombined `+371.6610`。00326 row x horizon + hpen0.25はactual-floorなしでcombined `+374.6110` まで伸びるが、00327のsupport-repair-only calibrationはprior不足で失敗。00328のbroad duration prior direct penaltyはcombined `+363.0870` で勝ち候補も削る。00329のbroad-prior low-complexity horizon-choice rankerはcombined `+403.2680` まで伸びるが、month/role-trade/side-share blockersが残る。 |
...
1. 00329 low-complexity horizon rankerへ、horizon/context別prior residualを使ったlower-bound scoreを入れる。例: `pred_pnl - k * residual_mae_by_horizon_context - tail_penalty`。
2. residual calibrationはtarget月より前だけで作る。horizon / side / combined_regime / session_regime / near_miss_bucketの階層を使い、疎なcontextはglobalへshrinkする。
...
5. tail強化だけではbestが悪化したため、tail probabilityはhard thresholdよりlower-bound score内の不確実性項として扱う。
