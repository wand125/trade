...
00322追加リスク: overlapping candidate clusterのraw利益をpolicy evidenceと誤読しない。non-overlap診断もfull stateful replayではない。score>=5 broad universeのtail-loss AUC改善だけで720mを再開しない。
...
1. 00322 s2 q90 + one-failed broad horizon viability outputをstateful support-repair replayへ接続する。
...
4. Side-balanced support overlayは、non-overlap / stateful replayを通してから評価する。
