...
1. `hybrid2025 2025-10 long` のようなpositive predicted PnL failureへ、tail/overestimate/context reliability診断を追加する。`ranker_pred_pnl < 0` vetoでは止まらない損失を本流にする。
2. Prior/OOB reliabilityをdirect scoreではなくhead selection / abstention / confidence reportとして使う。fresh03で見えたPnL / beats60 signalとtail-loss calibrationを分離し、tail penaltyはtrain countや単純multiplierではなく過去OOF信頼性の診断として扱う。
3. `fresh2024 2024-11` と `refit2025 2025-03` の候補生成pathを作る。00335 stateful surfaceに存在しないtargetをrerankingで解こうとしない。
4. 別family / 別期間 / 追加support-repair surfaceでsingleton事例を増やし、00338の`singleton_720_pred_pnl_lt2`をpre-registered diagnostic guardとして再検証する。
5. harmful probabilityをhorizon / side / session / regime / support bucket別にcalibrateし、global scalar penaltyや単純なharmful-lower switch ruleとして使わない。
6. 00329/00335 leak-free bestのremaining weak months、特に `fresh2024 2024-03`, `fresh2024 2024-08`, `fresh2024 2024-11`, `refit2025 2025-03` を00331..00346 target labelsで診断する。単純fallback緩和、global early-support relaxation、train-count gate、reliability direct multiplier、stateful pred-pnl negative vetoのpolicy化は本流にしない。
