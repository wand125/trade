...
1. `singleton_720_pred_pnl_lt2` はnarrow diagnostic guardとして保持し、追加support-repair surface / chronologyで再検証する。標準policyにはしない。
...
4. 別family / 別期間 / 追加support-repair surfaceでsingleton事例を増やし、00358の`singleton_720_pred_pnl_lt2`をpre-registered diagnostic guardとして再検証する。
