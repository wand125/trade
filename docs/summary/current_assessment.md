...
1. `holdext_long_range_normal_ny` をpost-hold no-replacement blockから、実行時proxyまたはmodel-used-aware extension vetoへ戻す。
2. 00308 branchをsupport-aware admission / strict NoTrade-first blockersで再評価する。
3. Short replacementは全family一律ではなく、family / regime / prior context supportで分割して再評価する。
4. `large_loss_uncompensated_by_context` は教師候補として残し、entry/exit sequence features、replacement state、skipped next winner costと組み合わせる。
5. large-loss probabilityとuncompensated-loss probabilityはhard gateではなく、candidate-level selector / stateful replay / exit timing targetの補助featureへ回す。
6. sequence-state診断の `next_*` はerror analysis専用にし、実行時featureは `prev_*`, 月内trade count, prior-only contextに限定する。
7. `isolated_large_loss_long + fixed720 + threshold -5 + replacement + require-model-used` はdiagnostic branchとして維持し、標準policyにはしない。
8. support-aware分類を今後の候補比較に使うが、通過だけで標準化しない。
9. `support_aware_only` 候補は標準policyにせず、unused chronologyまたはmodel-level calibration reasonが出るまでNoTradeを維持する。
