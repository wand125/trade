...
| Current diagnostic candidate | q95 + `loss_exit30` dynamic exit。`direction_regime` tail-riskはdiagnostic featureへ降格 |
| Why not standard | `00276` の統合では全role positiveだがmonth min `-4.1460` が残る。HGBで通った `loss_exit20/25` は同window sweep由来で、hybridとの閾値台地もずれる |
...
| Component / exit-regret | `00240`..`00276` | EV overestimateからdirection/exit/replacementへ分解。00267でq99 prior guardがstateful replay上は改善したが、標準admission未通過。00268でfresh support不足がepisode集中であり、rank0緩和はcal/refitを壊すと確認。00269の外部HGB、00270の外部full-hybridでもNoTrade未満。00271で損失はno-edgeではなくexit-capture failure / executable EV過大評価に寄ると確認。00272でpost-selector executable scoreは負の対照としてreject。00273でselector前capture補正もNoTrade未満。00274でcoarse `direction_regime` tail-riskはq99をプラス化したが、support/side集中でNoTrade。00275で外部HGB再現は弱く、tail-risk headはdiagnosticへ降格。00276でlow loss-first dynamic exitが強い改善を見せたが、month floorとthreshold scale driftが残る。 |
...
3. `loss_exit30` の改善をtrade deltaで分解し、早期決済そのものか追加entry機会かを切り分ける。
