...
1. stateful selection前の広いgated support候補を非重複decision clusterへ展開し、selected-addition近傍だけに依存しないlistwise repair utility targetを作る。
2. harmful probabilityをhorizon / side / session / regime / support bucket別にcalibrateし、global scalar penaltyや単純なharmful-lower switch ruleとして使わない。
3. 00329 bestのremaining weak months、特に `fresh2024 2024-03`, `fresh2024 2024-11`, `refit2025 2025-03` を00331/00332/00333 target labelsで診断する。単純fallback緩和は本流にしない。
4. `role_trades_low` と `side_share_high` を解く候補は、non-negative floor contributionを必須にし、EV-2の負け候補をsupport目的だけで入れない。
5. thin month / opposite-side supportを目的関数に明示し、単純なhorizon EV最大化だけでrepair targetが不変になる状態を避ける。
6. actual-floorは診断専用に残し、実行可能policyではobservable / learned proxyで代替する。
7. fallback0.25とfallback-allowed rankerはdiagnostic sensitivityとして維持し、learned evidenceや標準policy値として扱わない。
