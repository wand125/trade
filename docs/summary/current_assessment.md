...
1. q95 + raw `loss_exit30_cd15` を固定benchmarkにしたまま、supervised shrinkage outputを補助featureとしてmeta selector / downside-weighted headへ入れる。
2. targetはscore replacementではなく、high downside / low realized-EV rowを止めるblocker確率やdownside quantileに寄せる。
3. prior capture factor、loss-first probability、predicted holding、side confidence gap、regime/session/family、fixed-horizon proxyを特徴量として使う。
4. no-edge entryはrare-event guardとして別枠にし、3件だけでcontext blacklistを作らない。
