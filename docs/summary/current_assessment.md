...
2. support-sufficient negative month laneは、current-branch negative repairとcross-artifact robustnessを分ける。00378でsupport-sufficient targetは増えたがearly-month prior不足が出たので、same-family priorがないtarget向けのreplacement calibrationを別laneにする。
