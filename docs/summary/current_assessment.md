...
1. `long_range_normal_ny_fixed60_pred_gt0` を未使用chronologyへ再探索なしで適用し、refit集中が再現するか確認する。
2. fixed60 positive predictionがactual short-horizon negativeになる型を、post-hoc actualではなく事前のuncertainty / calibration residual featureへ戻す。
