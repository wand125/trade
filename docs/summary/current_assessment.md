...
`00334` でstateful selection直前の広いgated候補面をlistwise clusterとして診断した。`selected + quota_full` rowsを再構成し、同じquotaと一玉非重複制約で `repair_score`, actual oracle, predicted PnL, low harmful, low tail, high support proxyのgreedy選択を比較した。baseline best scenarioは31候補まで広がったが、`repair_score_greedy` はcurrent replayと一致し、actual oracle差は `+2.6360` のみ。EV -2 scenarioは111候補まで広がりactual oracle差 `+22.3190` だが、`fresh2024 2024-08 long -29.1360` は候補が1本しかなくrerankingでは救えない。listwise cluster診断インフラはaccepted。simple rerankerでは標準blockerを解けないため、次はlistwise examplesを教師化し、fresh/thin monthの候補生成またはabstention層へ進む。
...
| Support repair continuation | `00323`..`00334` | 00322 s2を00314 best branchのsupport repairへ接続。00323 best totalは5本追加 / `+23.4090` / combined `+362.7000`。00325 actual-floor upper-boundはcombined `+371.6610`。00326 row x horizon + hpen0.25はactual-floorなしでcombined `+374.6110` まで伸びるが、00327のsupport-repair-only calibrationはprior不足で失敗。00328のbroad duration prior direct penaltyはcombined `+363.0870` で勝ち候補も削る。00329のbroad-prior low-complexity horizon-choice rankerはcombined `+403.2680` まで伸びるが、month/role-trade/side-share blockersが残る。00330 lower-bound、00331 harmful direct penalty、00332 support-aware harmful penaltyはbaselineを超えない。00333/00334でswitch/listwise候補面を診断し、baseline bestはreranking余地が小さく、EV -2のfresh唯一負け候補はrerankingでは救えないと確認。次はlistwise target教師化とfresh/thin monthの候補生成またはabstentionへ進む。 |
...
1. 00334のlistwise examplesをchronological training targetへ変換し、`actual_oracle_greedy_selected` やquota group内actual rankを教師にした低容量meta-selectorを作る。
...
4. 00329 bestのremaining weak months、特に `fresh2024 2024-03`, `fresh2024 2024-11`, `refit2025 2025-03` を00331/00332/00333/00334 target labelsで診断する。単純fallback緩和は本流にしない。
