...
| Support repair continuation | `00323`..`00330` | 00322 s2を00314 best branchのsupport repairへ接続。00323 best totalは5本追加 / `+23.4090` / combined `+362.7000`。00325 actual-floor upper-boundはcombined `+371.6610`。00326 row x horizon + hpen0.25はactual-floorなしでcombined `+374.6110` まで伸びるが、00327のsupport-repair-only calibrationはprior不足で失敗。00328のbroad duration prior direct penaltyはcombined `+363.0870` で勝ち候補も削る。00329のbroad-prior low-complexity horizon-choice rankerはcombined `+403.2680` まで伸びるが、month/role-trade/side-share blockersが残る。00330のlower-bound residual scoreはbaselineを超えず、residual priorは診断/featuresとして残す。 |
...
1. 00330 residual priorを診断/featuresとして残し、harmful overestimate と profitable high-variance 720m を分離するtargetを作る。
2. targeted harmful-overestimate labelは、model overestimate、chosen horizonの60m/target hurdle下回り、month/role support未補償を分けて設計する。
3. 00329 bestのremaining weak months、特に `fresh2024 2024-03`, `fresh2024 2024-11`, `refit2025 2025-03` をsupport-targeted coverageで診断する。単純fallback緩和は本流にしない。
