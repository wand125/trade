...
1. q95 + raw `loss_exit30_cd15` を固定benchmarkにしたまま、selected-trade shrinkage headをcandidate-row / dense side-row featureへ戻す。
2. prior capture factor、loss-first probability、predicted holding、side confidence gap、regime/session/family、fixed-horizon proxyを特徴量として使い、stateful replayで確認する。
