...
したがって、次の改善は「holding capの再探索」でも「2025系列へのshort hook追加」でもなく、entry EV calibration、rank/quantile admission control、NoTrade-first selectorを、複数validation window / purged walk-forward / regime別安定性評価へ拡張することを優先する。

## 次に検証すべきこと

1. Entry admission selectorを複数chronological validation windowへ拡張する。`2ヶ月validation -> 10ヶ月test` の単発選択では、support gateを通ってもregime外挿に失敗した。
...
4. raw EV絶対値ではなく、side/regime別のrank、calibrated EV quantile、support-aware thresholdをadmission特徴として評価し、複数validation windowでtotal、worst、trade support、side balanceがNoTradeを超えるかを見る。
