...
| Support repair continuation | `00323`..`00326` | 00322 s2を00314 best branchのsupport repairへ接続。00323 best totalは5本追加 / `+23.4090` / combined `+362.7000`。00325 actual-floor upper-boundはcombined `+371.6610`。00326 row x horizon + hpen0.25はactual-floorなしでcombined `+374.6110` まで伸びるが、same-set duration penaltyでありstandard blockersは残る。 |
...
- horizon duration penalty diagnostics
...
1. duration risk / horizon choiceをchronological OOFでcalibrateする。hpen0.25を固定値として採用せず、過去月だけで学習・選択する。
2. horizon別に `support_reduction_value + predicted_pnl - tail_penalty - duration_risk` を評価し、fresh2024 2024-08の悪い720m型を止められるか確認する。
