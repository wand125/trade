...
| Support repair continuation | `00323`..`00327` | 00322 s2を00314 best branchのsupport repairへ接続。00323 best totalは5本追加 / `+23.4090` / combined `+362.7000`。00325 actual-floor upper-boundはcombined `+371.6610`。00326 row x horizon + hpen0.25はactual-floorなしでcombined `+374.6110` まで伸びるが、00327のchronological calibrationではsupport-repair-only prior不足によりno-penalty相当 `+342.5250` へ戻った。fallback0.25は診断再現のみで、standard blockersは残る。 |
...
1. duration risk / short-horizon-overestimateを00322 broad candidate universeなど広いprior dataで学習し、support-repair対象行だけの疎なpriorに依存しない。
2. horizon別に `support_reduction_value + predicted_pnl - tail_penalty - learned_duration_risk` を評価し、fresh2024 2024-08の悪い720m型をtarget月blindに止められるか確認する。
3. fallback0.25はdiagnostic sensitivityとして維持し、learned evidenceや標準policy値として扱わない。
4. actual-floorは診断専用に残し、実行可能policyではobservable / learned proxyで代替する。
5. `fresh2024 2024-03` はthreshold緩和ではなく、fallback/non-model clusterを分離するtarget-local confidence featureを先に作る。
6. `fresh2024 2024-11` は候補coverageが1本だけなので、candidate universe拡張かhorizon-choice calibration guardを先に行う。
7. refit2025 2025-07は、side-share修復だけを目的にEV-2の負け候補を入れない。non-negative floor contributionを必須にする。
8. 720m tail-loss classifierとPnL regressorのcalibrationを優先し、悪い720m pathを止められるまでsupport overlayへ接続しない。
