...
| Support repair continuation | `00323`..`00328` | 00322 s2を00314 best branchのsupport repairへ接続。00323 best totalは5本追加 / `+23.4090` / combined `+362.7000`。00325 actual-floor upper-boundはcombined `+371.6610`。00326 row x horizon + hpen0.25はactual-floorなしでcombined `+374.6110` まで伸びるが、00327のsupport-repair-only calibrationはprior不足で失敗。00328のbroad duration priorは2024-08の悪い720mを事前警告できるが、direct penalty bestはcombined `+363.0870` で勝ち候補も削る。 |
...
1. Broad duration priorを静的penaltyではなく、chronological horizon-choice ranker/headのfeatureにする。
2. Horizon-choice headは `fixed_horizon_pnl`, `fixed_horizon > fixed60`, `tail_loss`, `opportunity_preservation` を分け、refit2025 2025-07のような勝ち240mを削らない設計にする。
3. horizon別に `support_reduction_value + predicted_pnl - tail_penalty - learned_duration_risk + opportunity_score` を評価し、fresh2024 2024-08の悪い720m型をtarget月blindに止められるか確認する。
4. fallback0.25はdiagnostic sensitivityとして維持し、learned evidenceや標準policy値として扱わない。
5. actual-floorは診断専用に残し、実行可能policyではobservable / learned proxyで代替する。
6. `fresh2024 2024-03` はthreshold緩和ではなく、fallback/non-model clusterを分離するtarget-local confidence featureを先に作る。
7. `fresh2024 2024-11` は候補coverageが1本だけなので、candidate universe拡張かhorizon-choice calibration guardを先に行う。
8. refit2025 2025-07は、side-share修復だけを目的にEV-2の負け候補を入れない。non-negative floor contributionを必須にする。
9. 720m tail-loss classifierとPnL regressorのcalibrationを優先し、悪い720m pathを止められるまでsupport overlayへ接続しない。
10. repair targetを候補比較の補助ゲートにする。totalが改善してもrepair targetが不変なら、標準化への前進とは扱わない。
11. Context-count shrinkageではなく、OOF calibrated EV uncertaintyをcontinuous featureとしてexpected PnLへ入れる。
12. entry-block候補は今後もholdout-support diagnosticsを通し、発火0件や1件支持ならhard blockへ昇格しない。
13. `large_loss_uncompensated_by_context` は教師候補として残し、entry/exit sequence features、replacement state、skipped next winner costと組み合わせる。
14. sequence-state診断の `next_*` はerror analysis専用にし、実行時featureは `prev_*`, 月内trade count, prior-only contextに限定する。
15. `isolated_large_loss_long + fixed720 + threshold -5 + replacement + require-model-used` はdiagnostic branchとして維持し、extension veto悪化とentry-time proxyのrefit集中も含めて標準policyにはしない。
16. role trade support、role PnL、month floor、side share、NoTrade-first比較を標準採用ゲートとして維持する。
