...
2. [Report Map](report_map.md)
   `00001` から `00265` までのレポートを研究レーン別に圧縮した地図。
...
最新の有望軸は `exit_regret_selector_confidenceexit_bucket_t0p4` と、そのreplacement guard replay。ただし `00262` のadmission gateでは strict / relaxed とも NoTrade。`00263` では fresh2024 0-trade の主因が post-block `side_gap_pct` 汚染だと分かり、`00264` でpre-block side-gap quantileを実装したが、`00265` でrefit only-candidate tailが大きいことを確認したため標準化しない。
