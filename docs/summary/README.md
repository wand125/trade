...
2. [Report Map](report_map.md)
   `00001` から `00200` までのレポート系列をテーマ別に圧縮した地図。
...
- `short` entry budget: repeated active short を制限する軸。`budget0` 追加で prior-only が大きく改善し、fixed `gap5 -> gap0` triggerもmin4..6では改善する。ただしmin8はNoTrade未満。00196..00200で、late common short、`gap5` replacement short、prior signal coverage、`range_low_vol/ny_overlap` のentry-level signal、dynamic hookを分解済み。focus ORはreplacement込みで悪化し、rank-onlyは小幅改善止まり。次はreplacement riskを目的変数化する。
