...
2. [Report Map](report_map.md)
   `00001` から `00197` までのレポート系列をテーマ別に圧縮した地図。
...
- `short` entry budget: repeated active short を制限する軸。`budget0` 追加で prior-only が大きく改善し、fixed `gap5 -> gap0` triggerもmin4..6では改善する。ただしmin8はNoTrade未満。alert context限定budget/admissionやfirst-loss capはglobal budget0系に届かない。00196/00197で、late windowのcommon shortと`gap5` replacement shortが主因だと分解済み。
