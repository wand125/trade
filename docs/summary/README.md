...
2. [Report Map](report_map.md)
   `00001` から `00251` までのレポートをテーマ別に圧縮した地図。
...
最新の焦点は entry EV 系の再整理。絶対EV閾値、rank gate、quantile admission、hold-cap延長、prior inversion guard、executable EV、side-balance、composite hard gateを順に試したが、標準候補はまだない。`00243` で `side_prior_pressure` がbaseよりAUC改善し、`00244` でprediction rowへ接続するとvalidationは改善した。ただし fixed 2025で崩れ、`00245` でcommon-entry lossとreplacement lossを分離し、`00246` でdirection-side inversionが次のprimary target候補になり、`00247` でprediction row接続、`00248` でselector/ranking feature診断、`00249` でreplacement positive-qualityとのcombined stateful replay、`00250` でq99 direction s0.1残存損失の分解、`00251` でexit-shortening targetのOOF診断まで進めた。hold-too-longは説明力があるが予測headとして弱く、標準policyはNoTradeのまま。
