...
|---|---|---|
| Latest decision | `00372` | winner-damage制約を入れると00371 surfaceは全滅。非oracleはwinner selectedが多く、oracleでもreplacement selectorがbaseline-positive月をnegativeへ反転させる。risk selectorだけでなくreplacement abstention/calibrationも必要。標準policyはNoTrade。 |
...
```text
Report: 00372 Entry EV Selector Surface Winner Damage
Status: accepted infrastructure / standard NoTrade
Question: winner-damage制約を入れると00371 surfaceで採用可能なrowは残るか
Best evidence: 16 surface rows中、loss precision >=0.5 / winner selected 0 / baseline-positive degradation 0 / current-negative delta >=0 を全て満たすものは0。oracleでも `hgb2024_0306 2024-05` が `+0.9578 -> -11.7730`。
Decision: winner-damage diagnosticsは採用。現risk selectorもreplacement selectorも標準policy化しない。
Next: winner-damage constrained objectiveをsurface rankingへ直接入れ、hgb2024_0306 2024-05 replacement failureを診断する
```
