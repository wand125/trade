...
```text
Report: 00351 Entry EV Contextual Risk Confidence Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: context-specific risk signalは過去月だけで信用できるか
Best evidence: market dedup後のdefaultでは confident context 0。min_prior_flagged=4に緩めると720m short/down_normal_vol/londonが勝ち候補36 rows / +465.8400だけをflag。
Decision: diagnosticsは採用。row-weighted prior supportと薄いexact-context hard gateはreject。context riskはhierarchical/shrunk featureへ回す
Next: prior_month_count / unique decision support、horizon/context別calibration、候補生成不足pathへ進む
```
