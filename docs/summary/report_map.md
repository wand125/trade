...
```text
Report: 00355 Entry EV Contextual Penalty Near-Selected Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: 00354のpenalized rowsは選択順位から遠いのか、それとも別のfilterで落ちているのか
Best evidence: 各contextual labelの656 penalized rowsは全件 tail_prob_ceiling でpre-filter reject。tail probはmin 0.312885 / median 0.381267で、max_chosen_tail_prob=0.3を全件超過。
Decision: contextual confidenceは現standard replayではtail ceilingの説明/監査signal。soft penalty強化は本流にしない
Next: tail ceiling通過後の残存failureとremaining weak monthsの候補生成不足へ戻る
```
