...
```text
Report: 00362 Entry EV Upstream Universe Coverage Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: refit2025 2025-03 shortが00318/00322で0件になる原因はraw prediction coverage不足か
Best evidence: raw prediction rows `28,972`, short side rows `28,972`, candidates `41`, available `33`。ただし `extra_short_needed=0` なので00318 targetが発行されない。
Decision: upstream coverage diagnosticsは採用。refit2025-03はsupport-sufficient negative monthのrepair-target objective mismatchとして扱う
Next: support-sufficient negative-month repair laneと、2024-11の狭いreplacement replay
```
