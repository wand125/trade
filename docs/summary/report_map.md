...
```text
Report: 00308 Entry EV Replacement Hold-Extension Integration
Status: accepted infrastructure / diagnostic candidate
Question: short entry-block replacement pathへside-aware hold-extensionを安全に戻せるか
Best evidence: replacement + require-model-used `isolated_large_loss_long / t-5 / h720` は total +326.1098 / month min -0.8832。さらに `holdext_long_range_normal_ny` blockで total +326.9930 / month min -0.7200。require-model-usedなしでは fallback fixed720 が hgb2024-03 を -17.6936 へ壊した
Decision: 標準policyはNoTrade
Next: holdext false-positive blockを実行時proxy / extension vetoへ戻す
```
