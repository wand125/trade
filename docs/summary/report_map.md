...
```text
Report: 00301 Entry EV Prior Residual Pressure
Status: accepted infrastructure / hard gate rejected
Question: 危険contextを対象月より前の履歴だけで検知できるか
Best evidence: factor fine-context best ruleは6 trades / flagged PnL -10.8380。ただしPnL modeでは悪化し、広いruleは勝ちを削る
Decision: 標準policyはNoTrade
Next: prior residual pressureをfeatureとしてuncertainty / large-loss headやcandidate-level selectorへ入れる
```
