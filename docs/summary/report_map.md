...
```text
Report: 00356 Entry EV Tail Ceiling Residual Failure Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: max_chosen_tail_prob=0.3通過後にも残るpositive predicted PnL failureは、global hard gateで削れるか
Best evidence: row-weighted positive predicted PnLはtail blocked側3236 rows / -47165.1376、tail pass側9308 rows / -120.6816。greedy selectedのtail pass positiveは82 rows / +500.7960。
Decision: tail ceilingは維持。tail pass residual global hard gateはwinner damageが大きいためreject
Next: selected/near-selected tail-pass residual failureをquota boundary、weak months、horizon/exit calibrationに分解する
```
