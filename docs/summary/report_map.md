...
|---|---|---|
| Latest decision | `00377` | aligned artifact surfaceで非oracle passは出たが、実質1 target改善でsupport-limited月はcandidate 0。support-sufficient repairとsupport-limited candidate generationを分ける。標準policyはNoTrade。 |
...
```text
Report: 00377 Entry EV Aligned Current-Negative Surface
Status: accepted infrastructure / standard NoTrade
Question: config合成した00310 aligned pathでcurrent-negative targetを複数評価できるか
Best evidence: 4 target評価で `side_gap` + `prior_actual_mean` はloss selected 3 / winner 0 / precision 1.0 / mean delta +5.0618。ただし改善は refit2025 2025-03 の +20.2470 だけで、support-limited月は候補不足。
Decision: aligned surfaceは採用。multi-target repair成功ではない。
Next: support-sufficient repair laneとsupport-limited candidate-generation laneを分ける
```
