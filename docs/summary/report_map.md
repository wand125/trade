...
```text
Report: 00305 Entry EV Uncompensated Sequence State
Status: accepted infrastructure / direct uncompensated-risk gate rejected
Question: uncompensated targetは孤立損失なのか、それともsequence pathに埋まっているのか
Best evidence: `pnl/base/base` のtarget 22件中、`>10` trade月が18件、次trade勝ちが15件、前回勝ち後が12件。threshold除去96本は全て悪化し、最小悪化でもflagged PnL +5.6900
Decision: 標準policyはNoTrade
Next: candidate-level selector / stateful replayでreplacement / skipped next winnerを評価する
```
