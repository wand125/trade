...
```text
Report: 00311 Entry EV Position-Quality Holdout Support
Status: accepted infrastructure / hard-block candidate rejected
Question: 00310の `long_range_normal_ny_fixed60_pred_gt0` は非refit holdoutで再探索なしに再現するか
Best evidence: `long_range_normal_ny_fixed60_pred_gt0` は全体 +11.4912 / discovery refit +11.4912 / holdout発火0件。broader `long_range_normal_ny` もholdout +0.7370だが1 winnerを削る
Decision: 標準policyはNoTrade
Next: short-horizon overestimateをuncertainty/calibration residual featureへ戻す
```
