...
| `00186`..`00197` | short-specific interaction / entry budget | short raw gapは介入箇所を示す。`budget0` とprior realized/context-alert composite triggerによりtailは大きく縮んだが、prediction/alert単独triggerは上積みできない。alert context限定budget/admission/first-lossは狭すぎる。00196/00197で、global budget0との差はlate common shortと`gap5` replacement shortの残存だと分解した。fixed triggerはmin4..6で効くがlate-onlyではNoTrade未満。 |
