...
```text
Report: 00286 Entry EV Stateful Floor Meta Selector
Status: accepted infrastructure / current candidates rejected
Question: 複数実験候補をstateful month/role floor込みで横断選択できるか
Best evidence: raw cd15 baseline and downside gte3 tie at total +118.6900 / role min +0.0074 / month min -6.8324; all candidates fail even floor-only gates
Decision: 標準policyはNoTrade
Next: raw cd15 losing monthsのexit timing / cooldown / post-exit re-entry pathを改善する
```
