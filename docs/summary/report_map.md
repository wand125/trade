...
```text
Report: 00352 Entry EV Context Support Count Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: context risk confidenceはsupport数を入れるとどう変わるか
Best evidence: exact contextは発火0。horizon,side support2のpositive_bias_and_tail_missは flagged PnL -39210.5520 / losses 1008 / wins 0 / selected flagged win 0。horizon,side,regime support2は発火0。
Decision: support columns/controlは採用。coarse positive-biasはstateful replay候補。harmful/residual/coarse tail hard gateはwinner damageでreject
Next: contextual positive-bias support2をstateful replayへ戻す
```
