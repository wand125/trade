...
```text
Report: 00361 Entry EV Selected Replacement Scope Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: selected one-fail行をreplacement候補scopeとして再露出できるか
Best evidence: 2024-11は1行出るがstrict不可/relaxedのみ。global selected-onefail wideningは00358 ranker strict `-83.4028`, relaxed `-68.0198`。
Decision: diagnostic scopeは採用、global wideningはreject。2024-11はtarget-aware replacement + horizon guardで扱う
Next: 2024-11の狭いreplacement replayと、refit2025-03のupstream universe監査
```
