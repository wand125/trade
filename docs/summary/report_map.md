...
```text
Report: 00343 Entry EV Prior OOB Reliability Horizon Choice
Status: accepted infrastructure / standard NoTrade
Question: prior/OOB reliabilityでhead信頼性を測ればdelta/tail scoringは改善するか
Best evidence: reliability-gated scoreはfull replayでplain pnlと同じ+400.1440に戻るが、candidate-levelでは2024-08/2025-07を悪化させる。
Decision: 標準policyはNoTrade
Next: reliabilityはdirect scoreではなくhead selection / confidence reportへ回し、候補生成へ戻る
```
