...
```text
Report: 00344 Entry EV Horizon Reliability Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: reliabilityをdirect scoreでなく診断すると、どこでhorizon選択を壊すか
Best evidence: available candidatesでpnl_delta_tail_reliability_gatedはtarget subset -131.8792、all rows -137.6916悪化。
Decision: 標準policyはNoTrade
Next: reliabilityをhead selection / abstention / confidence reportへ回し、候補生成へ戻る
```
