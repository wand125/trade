...
```text
Report: 00306 Entry EV Uncompensated Candidate Path
Status: accepted infrastructure / target-count minimization rejected
Question: uncompensated targetを減らす候補が本当に良いcandidate pathなのか
Best evidence: 00293 best branchはtarget 22件が残るが total +329.4348 / month min -0.7200。target countとtotal PnLの相関は +0.0502、target countとmonth floorの相関は +0.5674
Decision: 標準policyはNoTrade
Next: 未選択entry候補feedを使うstateful replacement replayへ進む
```
