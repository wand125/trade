...
```text
Report: 00302 Entry EV Prior Pressure Large Loss Head
Status: accepted infrastructure / prior feature and direct risk gate rejected
Question: prior residual pressureはlarge-loss headのfeatureとして効くか
Best evidence: base PnL AP 0.2146に対しbase_prior AP 0.1604。high-risk除去は最小でもflagged PnL +15.0000で悪化
Decision: 標準policyはNoTrade
Next: pointwise gateを増やさず、candidate-level selector / stateful replay / path-aware labelへ戻す
```
