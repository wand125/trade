...
|---|---|---|
| Latest decision | `00366` | 00365のbroad horizon-abstention ruleは、`all/predicted` の探索的argmaxでは有効だが、既存本線 `isolated_large_loss_long/fixed720/t-5` では全8 good extensionsを止めてraw baseへ戻す。extension veto hookはaccepted infrastructureだが、同ruleは本線vetoとしてreject。次はsupport-sufficient negative month向けにreplacement selector / expected PnL calibrationへ戻る。 |
...
```text
Report: 00366 Entry EV Horizon Abstention Stateful Replay
Status: accepted infrastructure / standard NoTrade
Question: 00365 broad abstention ruleはone-position制約込みでも使えるか
Best evidence: `all/predicted/t-5` は rule なし `+52.4794`, month min `-324.7062` から rule あり `+267.8748`, month min `-32.9086` へ改善。一方、本線 `isolated_large_loss_long/fixed720/t-5` は rule なし `+338.4078`, month min `-0.8832` がbestで、ruleありは全8 extensionsを止め raw base `+139.1098`, month min `-6.8324` へ戻る。
Decision: extension veto hookは採用。00365 broad ruleは探索的predicted-horizon安全診断として残すが、本線hold-extension vetoとしてはreject。
Next: support-sufficient negative month向けにreplacement selector / expected PnL calibrationへ戻る
```
