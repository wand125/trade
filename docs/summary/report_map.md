...
|---|---|---|
| Latest decision | `00367` | support-sufficient negative month向けにprior-calibrated replacement rankingを診断した。min50では `prior_actual_mean` がmean month PnL `+18.3040`, best `+19.7740`、`bias_corrected` がmean `+8.0640`。ただしpriorは2ヶ月、選択候補は全てone-failなので標準policyではない。次はprior month/support thresholdをselector surface化し、loss-risk selectorと接続する。 |
...
```text
Report: 00367 Entry EV Support-Sufficient Replacement Calibration
Status: accepted infrastructure / standard NoTrade
Question: support-sufficient negative monthのreplacement候補をprior-calibrated expected PnLでrankできるか
Best evidence: min50では `prior_actual_mean` がmean month PnL `+18.3040`, best `+19.7740`; `bias_corrected` がmean `+8.0640`。min20 `bias_corrected` はmean `+22.1670` だが、min20 downside/conservativeは `-18.7634` へ悪化。
Decision: diagnosticsは採用。prior_actual_mean/bias_correctedは有望featureだが、prior 2ヶ月 + one-fail依存のため標準policyではない。
Next: prior month/support thresholdをselector surface化し、loss-risk selectorと組み合わせる
```
