...
```text
Report: 00296 Entry EV Support-Aware Progression Compare
Status: accepted infrastructure / diagnostic candidate remains NoTrade
Question: support-aware分類で候補系列の進歩は安定して見えるか
Best evidence: defaultでsupport_aware_onlyになるのは00293 residual comboだけ。ただしsupport2/shallow025ではblocked
Decision: 標準policyはNoTrade
Next: support-aware分類は診断層に留め、unused chronologyかpolicy-level mechanismを待つ
```
