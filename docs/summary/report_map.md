...
```text
Report: 00310 Entry EV Position-Quality Proxy Overlay
Status: accepted infrastructure / diagnostic candidate
Question: 00309でvetoできなかった `holdext_long_range_normal_ny` false positiveをentry-time observableなposition-quality proxyで扱えるか
Best evidence: `long_range_normal_ny_fixed60_pred_gt0` は `isolated_large_loss_long / t-5 / h720` を total +326.1098 / month min -0.8832 から +337.6010 / -0.7200 へ改善。ただしblocked 4件は全てrefit2025_validationで、support2/shallow025ではblocked
Decision: 標準policyはNoTrade
Next: 未使用chronologyへ再探索なし適用し、short-horizon overestimateをuncertainty/calibration residual featureへ戻す
```
