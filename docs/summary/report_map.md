...
| `00186`..`00198` | short-specific interaction / entry budget | short raw gapは介入箇所を示す。`budget0` とprior realized/context-alert composite triggerによりtailは大きく縮んだが、prediction/alert単独triggerは上積みできない。alert context限定budget/admission/first-lossは狭すぎる。00196..00198で、global budget0との差、`gap5` replacement short、prior signal coverageを分解した。context alert強化だけでは `range_low_vol/ny_overlap` が残る。 |
