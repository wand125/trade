...
```text
Report: 00281 Entry EV Capture Shrink Overlay
Status: pre-standard diagnostic candidate / policy not standardized
Question: prior exit-capture risk / capture factorをhard blockまたはdirect score shrinkとして使えるか
Best evidence: full side/regime/session shrink bestは +10.8662 でraw +118.6900を大きく下回る。weak shrink 0.25も -0.2380 / role min -16.2258
Decision: 標準policyはNoTrade
Next: capture factorを直接掛けず、supervised exit-capture / realized-PnL shrinkage headの特徴量として使う
```
