...
```text
Report: 00348 Entry EV Positive PnL Gate Stateful Replay
Status: accepted infrastructure / standard NoTrade
Question: positive predicted PnL failure ruleをhard gateにするとstateful bestは改善するか
Best evidence: selector pass 0/864。tail_prob_ge_0p30はbest no-op、positive_bias_and_tail_miss_ge_0p10はcombined +400.1440 -> +393.2940へ悪化。
Decision: positive-PnL gate replay infraは採用。両ruleはhard gateとしてreject
Next: horizon/context別calibration、soft penalty、over-gating diagnosticsへ進む
```
