...
```text
Report: 00357 Entry EV Tail Selected Residual Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: tail ceiling通過後のresidual failureはactual selected / near-selectedにどれだけ残るか
Best evidence: candidate identity selected additionsのtail pass positiveは8件 / +59.0070、lossは1件 / -29.1360。`pred_pnl_lt_2` はその1件だけをflagしwin damage 0。
Decision: tail selected residual diagnosticsはaccepted infrastructure。`pred_pnl_lt_2` はunique 1件支持なので標準policyにはしない
Next: `selected tail-pass pred_pnl_lt2` と `singleton_720_pred_pnl_lt2` をstateful replayへ戻す
```
