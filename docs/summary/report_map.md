...
```text
Report: 00346 Entry EV Stateful Reliability Abstention Replay
Status: accepted infrastructure / standard NoTrade
Question: ranker_pred_pnl < 0 switch vetoはstateful replayでもbestを改善するか
Best evidence: vetoありでもbestはplain pnlと同じ5 trades / added PnL +60.8530 / combined +400.1440、selector pass 0件。
Decision: abstention replay infrastructureは残すが、pred_pnl_lt0 switch vetoは標準policyへ昇格しない
Next: positive predicted PnL failure向けtail/overestimate/context reliability診断と、候補生成不足targetのcandidate pathを進める
```
