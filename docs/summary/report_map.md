...
```text
Report: 00364 Entry EV Support-Sufficient Loss Risk Prior
Status: accepted infrastructure / standard NoTrade
Question: support-sufficient negative monthのrefit2025-03 lossを実行時特徴と時系列priorで拾えるか
Best evidence: targetでは `lossfirst_ge0p40_or_ev_ge5_lossfirst_lt0p30` が4/4 lossを捕捉し target flagged PnL `-1.5900`。ただし全体では176 trades / flagged PnL `+332.3394` でwinner damageが大きい。
Decision: loss-risk prior diagnosticsは採用。direct block / hard gateはreject。
Next: replacement selector、horizon abstention、expected PnL calibrationの補助featureとして使う
```
