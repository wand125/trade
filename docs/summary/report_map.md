...
```text
Report: 00285 Entry EV Downside Meta Risk Margin
Status: accepted infrastructure / score margin rejected
Question: raw cd15 scoreにdownside meta soft marginを入れるとtailを守れるか
Best evidence: best total w0.25は +23.7938 でbaseline +118.6900を大きく下回り、w1もmonth min小改善と引き換えにrole min -3.9590
Decision: 標準policyはNoTrade
Next: score arithmeticを止め、candidate-level stateful floor-aware meta selectorへ進む
```
