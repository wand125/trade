...
```text
Report: 00349 Entry EV Positive PnL Soft Penalty Replay
Status: accepted infrastructure / standard NoTrade
Question: positive-PnL failure signalをsoft penaltyにすればstateful bestは改善するか
Best evidence: selector pass 0/2016。residual_bias_tail_miss w0.05..0.25 と tail_prob w1/w2 はbest no-op、tail_prob w5はcombined +399.8040へ悪化。
Decision: positive-PnL soft penalty infraは採用。global soft penaltyはpolicy candidateとしてreject
Next: over-gating diagnostics、horizon/context別calibration、lower-bound EVへ進む
```
