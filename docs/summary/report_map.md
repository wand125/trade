...
```text
Report: 00312 Entry EV Fixed60 Prior Uncertainty
Status: accepted infrastructure / hard-gate candidate rejected
Question: fixed60 positive predictionがactual short-horizon negativeになる型を、対象月より前のprior featureで事前に表現できるか
Best evidence: fine context `prior_count_ge5_pnl_neg_fp_rate_ge0p4` は4 trades / flagged PnL -11.4360 / final loss precision 1.0。ただし全てrefit discoveryで、holdout発火0件
Decision: 標準policyはNoTrade
Next: `prior_fixed_*` をcandidate-level selector / uncertainty headのfeatureとして検証する
```
