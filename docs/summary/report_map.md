...
```text
Report: 00345 Entry EV Horizon Reliability Abstention Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: reliability-driven horizon switchをprediction-only vetoで戻す余地はあるか
Best evidence: ranker_pred_pnl < 0 vetoでpnl_delta_tail_reliability_gatedはtarget subset -131.8792 -> +13.6962、all rows -137.6916 -> +57.0582。
Decision: 標準policyはNoTrade
Next: pred_pnl_lt0 switch vetoをstateful replayへ実装し、positive pred PnL failure向けtail/overestimate診断を追加する
```
