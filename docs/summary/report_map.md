...
```text
Report: 00353 Entry EV Context H/S Support2 Positive PnL Gate Replay
Status: accepted infrastructure / standard NoTrade
Question: 00352のhorizon,side support2 positive-bias signalはstateful replay上でも最終PnLを改善するか
Best evidence: 候補段階では各score/abstentionで82 rowsをvetoし vetoed PnL -1222.4120 / loss 78 / win 4。ただし最終summary差分は0/288でbest combinedは+400.1440のまま。
Decision: contextual gate hookは採用。hard prefilterとしては標準化しない
Next: contextual confidenceをrepair score / listwise selector feature / admission explanationへ回す
```
