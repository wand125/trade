...
```text
Report: 00350 Entry EV Over-Gating Context Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: risk ruleは損失捕捉よりselected winner damageが大きくないか
Best evidence: best scenarioで tail_prob_ge_0p30 は発火0。harmful_prob_ge_0p30 は selected flagged win PnL +49.5600、residual_tail_miss_ge_0p10 は selected winners 5本 / +60.8530 を全て巻き込む。
Decision: over-gating diagnosticsは採用。global harmful/residual/720m gateはreject。tail probabilityはcontext-specific risk priorとして扱う
Next: context-specific abstention confidence、horizon/context別calibration、候補生成不足pathへ進む
```
