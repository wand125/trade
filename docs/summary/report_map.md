...
```text
Report: 00303 Entry EV Path Compensation Diagnostics
Status: accepted infrastructure / direct risk gate rejected
Question: large-loss risk scoreは同context-monthの勝ちに補償されるpathを区別できているか
Best evidence: compensated large lossは1/23のみだが、risk threshold除去は20本すべて悪化。最小悪化でもflagged PnL +15.0000
Decision: 標準policyはNoTrade
Next: `large_loss_uncompensated_by_context` / negative path contextを教師候補にし、candidate-level selector / stateful replayへ戻す
```
