...
```text
Report: 00295 Entry EV Support-Aware Admission
Status: accepted infrastructure / diagnostic candidate remains NoTrade
Question: 月次floor breachをsupport-limited / shallow / structuralに分けると00293 bestはどう見えるか
Best evidence: defaultではsupport_aware_onlyだが、support-limited許容2またはshallow floor -0.25ではblocked。strict blockersは残る
Decision: 標準policyはNoTrade
Next: support-aware分類を追加chronologyの候補比較に使う
```
