...
47. `00233_2026-06-30_entry_ev_side_balance_score_penalty.md`
48. `00234_2026-06-30_entry_ev_side_balance_feature_diagnostics.md`
49. `00235_2026-06-30_entry_ev_side_balance_downside_interaction.md`
50. `00236_2026-06-30_entry_ev_side_balance_downside_selector.md`
...
```text
Report: 00227 Entry EV Residual 2024-03 Loss Diagnostics
Status: accepted diagnostic / not standard
Question: q95_floor5/720mで残るfresh 2024-03の月次損失はentry不足か、direction/exit/calibration不足か。
Best evidence: 18 trades total -9.1718. All 18 have same-side oracle edge, same-side oracle total +327.9840, no_edge_entry 0. Loss trades 7 total -52.0548; direction error 7 trades / -46.3626 and large exit regret 13 trades / -30.5188.
Decision: 残差月はentry floor不足ではない。direction-side inversion, exit capture, realized executable EV calibrationへ分解する。
Next: targetize exit-capture failure and calibrate predicted oracle EV by executable capture ratio.
```

```text
Report: 00228 Entry EV Exit Capture Target Diagnostics
Status: accepted target/feature infrastructure / hard block rejected
Question: same-side oracle利益を実現できないtradeをtarget化し、prior-only risk scoreで拾えるか。
Best evidence: validation q95/q99 exit_capture_failure rate is high: refit q95/q99 0.8621..0.8929 and fresh q95 0.8421. However risk>=0.20 removes 68 trades / -23.1116 in validation but removes 77 trades / +225.3034 in fresh q95/720 fixed.
Decision: exit-capture target is useful for training/diagnostics, but prior exit risk hard block is rejected.
Next: use exit-capture labels for executable EV calibration and exit timing/capture modeling, not direct blocking.
```

```text
Report: 00229 Entry EV Executable EV Calibration
Status: accepted continuous feature / hard threshold rejected
Question: raw predicted EVを、現exit policyで実現可能なcapture factorで割り引くと過大評価は縮むか。
Best evidence: non-negative capture factor [0,1] reduces validation q95/q99 MAE: refit 20.8969..22.0208 -> 7.3980..7.6244, fresh q95 13.9256 -> 6.1870. Fresh q95/720 also improves validation 14.7507 -> 8.4237 and fixed 13.4582 -> 7.0417.
Decision: executable EV calibration is accepted as a continuous feature. Threshold EV<2/3/5 is unstable across windows and rejected.
Next: use pred_capture_calibrated_ev as selector/ranking/score feature without hard thresholding.
```

```text
Report: 00230 Entry EV Executable EV Selector Feature
Status: accepted diagnostic feature / not standard
Question: executable EVをcandidate-level NoTrade-first selector featureとして戻すと候補昇格できるか。
Best evidence: validation q95/q99 q99 rows have cleaner capture_ev_mean > 5, but still fail refit/month PnL gates. Fresh q95/720 has validation total +76.2204 and fixed total +325.8914, but validation min month -9.1718.
Decision: feature cleanliness cannot override NoTrade-first PnL gates. Current standard remains NoTrade.
Next: move executable EV into stateful entry score / replacement choice rather than post-trade selector features.
```

```text
Report: 00231 Entry EV Executable EV Stateful Score
Status: accepted input/score infrastructure / tested policies not standard
Question: executable EVをprediction全行のlong/short scoreへ入れ、stateful timed_evの実entry pathを変えるとNoTradeを超えるか。
...
```text
Report: 00233 Entry EV Side Balance Score Penalty
Status: accepted side-balance infrastructure / generic direct penalty not standard
Question: prior predicted-vs-target side share driftでdense executable scoreの過剰sideを縮小すると、refit long-heavy failureを直せるか。
Best evidence: refit2025 long share improves from 0.9453 -> 0.8911 in 2025-01 and 0.8970 -> 0.8750 in 2025-02; refit q95 floor5 becomes +93.9912 with min month +9.5300. However fresh q95 floor5 is -82.2428, q99 floor5 is -38.3550, and overall q95 floor5 still has min role -82.2428 / min month -46.5308.
Decision: direct side-balance penalty is rejected. It helps one visible side-drift mode but worsens fresh validation tail.
Next: use side-balance drift and side scales as selector/ranking/downside-conditioned features, not unconditional score multipliers.
```

```text
Report: 00234 Entry EV Side Balance Feature Diagnostics
Status: accepted selected-trade diagnostics / side-balance-only screens not standard
Question: side-balance driftをselected-trade featureとして見ると、負けtradeを安定して識別できるか。
Best evidence: fresh q95 floor5 is -82.2428 with overrep share 0.3750, while refit q95 floor5 is +93.9912 with overrep share 0.4737. q99 floor5 selected_underrepresented>=0.02 removes 11 trades / -16.8394 and keeps +6.6108, but q95 floor5 selected_underrepresented>=0.05 removes +14.0236 profit and selected_overrepresented>=0.05 removes +35.3140 profit.
Decision: side-balance drift alone is not a stable blocker or rank feature. q99 post-hoc improvement does not generalize to q95 and does not model stateful replacement.
Next: combine side-balance drift with prior side PnL, direction error, exit capture failure, context loss, and realized executable EV.
```
