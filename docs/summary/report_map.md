...
```text
Report: 00347 Entry EV Positive Predicted PnL Failure Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: positive predicted PnL候補は実損益上どれだけ信用できるか
Best evidence: market dedup positive pred 205件中124件が損失、合計 -1104.5216。positive_bias_and_tail_miss_ge_0p10 はprecision 0.7706 / recall 0.6774。
Decision: positive pred PnLをそのままadmission根拠にしない。pointwise gateは未採用
Next: positive_bias_and_tail_miss_ge_0p10 / tail_prob_ge_0p30 をstateful replay sensitivityへ入れる
```
