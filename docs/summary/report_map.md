...
```text
Report: 00304 Entry EV Uncompensated Loss Head
Status: accepted infrastructure / direct uncompensated-risk gate rejected
Question: `large_loss_uncompensated_by_context` を教師にすればpositive pathを分離できるか
Best evidence: best AP 0.1463で00302 large-loss AP 0.2146より低い。threshold除去160本すべて悪化し、最小悪化でもflagged PnL +5.6900
Decision: 標準policyはNoTrade
Next: pointwise classifierを増やさず、candidate-level selector / stateful replayへ戻す
```
