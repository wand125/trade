...
|---|---|---|
| Latest decision | `00380` | selector surfaceの合否はtarget-level outcomeで読む。00378の非oracle passは修復成功1件に対しcandidate gap 3件/risk gap 1件で、標準policyはNoTrade。 |
...
```text
Report: 00380 Entry EV Surface Target Outcome Diagnostics
Status: accepted infrastructure / standard NoTrade
Question: surfaceの制約通過をtarget単位で正しく読めているか
Best evidence: 非oracle ev_ge5 + prior_actual_meanは5 target中、修復成功1、candidate gap 3、risk gap 1。mean afterは -0.3246 のまま。
Decision: outcome categoryはaccepted infrastructure。loss selectedをrepair成功と誤読しない。
Next: outcome categoryをsurface ranking制約へ組み込み、shrunk cross-family priorを同じ分類で評価する
```
