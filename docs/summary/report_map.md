...
```text
Report: 00365 Entry EV Support-Sufficient Horizon Abstention
Status: accepted infrastructure / standard NoTrade
Question: predicted fixed-horizon argmaxをいつ信じないべきか
Best evidence: all selected tradesではfollow-all extension delta `-221.4806`。`lossfirst_ge0p40_or_pred_best_ge5_or_ev_lowlf` でabstainすると `+207.3556`。targetでは harmful horizon 7/7を止め、extension delta `-50.4340 -> +26.1200`。
Decision: horizon abstention diagnosticsは採用。candidate ruleはstateful replayへ進めるが標準policyではない。
Next: abstention ruleをhold-extension / predicted fixed-horizon replayへ接続し、one-position constraint下で検証する
```
