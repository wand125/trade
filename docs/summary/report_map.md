...
```text
Report: 00309 Entry EV Hold-Extension Veto Diagnostics
Status: accepted infrastructure / veto proxy rejected
Question: 00308のpost-hold `holdext_long_range_normal_ny` blockを実行時extension vetoへ戻せるか
Best evidence: 00308 bestはdefault support-awareでは support_aware_only だがsupport2/shallow025でblocked。extension vetoは `isolated_large_loss_long / t-5 / h720` を total +326.1098 / month min -0.8832 から +325.2078 / -1.7852 へ悪化
Decision: 標準policyはNoTrade
Next: entry-time observableなposition-quality proxyとして再検証する
```
