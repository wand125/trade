...
```text
Report: 00363 Entry EV Support-Sufficient Negative Month Repair
Status: accepted infrastructure / standard NoTrade
Question: support-sufficient negative monthのrefit2025-03は既存trade修復で直せるか
Best evidence: baseline `-0.4730`; oracle single fixed-best exit `+4.1230`, top-score replacement `+4.2170`; predicted fixed-horizon argmaxは `-8.8574` へ悪化。
Decision: 診断infraは採用。現predicted fixed-horizon argmaxはreject。loss-risk + horizon abstention + target-aware replacementへ進む
Next: loss-risk classifier、horizon abstention、target-aware replacement selector
```
