...
```text
Report: 00282 Entry EV Selected Trade Supervised Shrinkage
Status: pre-standard diagnostic candidate / policy not standardized
Question: selected raw cd15 trades上でsupervised realized-PnL shrinkageを直接gateに使えるか
Best evidence: supervised factor EVはMAE 2.2447でraw EV MAE 10.3692より良いが、threshold 0.0 removalは +46.2508 の勝ちPnLを削る
Decision: 標準policyはNoTrade
Next: shrinkage headをstandalone gateではなくcandidate-row / dense side-row featureへ戻す
```
