...
```text
Report: 00354 Entry EV Contextual Positive PnL Soft Penalty Replay
Status: accepted infrastructure / standard NoTrade
Question: 00353のcontextual confidenceをsoft repair penaltyにすると最終採用が変わるか
Best evidence: 候補段階では各labelで656 rows / penalized PnL -9779.2960を捕まえるが、selected additions with penalty > 0 は0。最終summary差分は0/288でbest combinedは+400.1440のまま。
Decision: contextual soft penalty modesは採用。scalar penaltyとしては標準化しない
Next: penalized rowsのquota group順位 / near-selected度を診断する
```
