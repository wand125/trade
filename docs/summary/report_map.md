...
```text
Report: 00329 Entry EV Broad Prior Horizon Choice Ranker
Status: accepted infrastructure / standard NoTrade
Question: broad duration priorを静的penaltyではなくhorizon-choice rankerのfeatureにするとsupport repairが改善するか
Best evidence: 低複雑度ranker(max_leaf_nodes=4,l2=5)は5本追加、added PnL +63.9770、combined +403.2680まで伸びた。ただしmonth min -0.6120、role trade min 3、side-share blockerが残る。
Decision: 標準policyはNoTrade
Next: horizon/context別prior residualからlower-bound scoreを作る
```
