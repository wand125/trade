...
- `reports/`
  実験ごとのレポートを置く。ファイル名は `00001_YYYY-MM-DD_slug.md` の通し番号形式にする。通し番号はファイルシステムの更新時刻(mtime)や本文の `更新日時` ではなく、レポート本文内の `日時: YYYY-MM-DD HH:MM JST` の昇順で決める。再採番・最新判断・既存レポート確認でも、必ずファイル内の `日時` を正とする。各レポートの冒頭には `日時: YYYY-MM-DD HH:MM JST` と `更新日時: YYYY-MM-DD HH:MM JST` を入れる。
