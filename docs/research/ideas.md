...
- fixed 2025 stressへdirection/exit residual targetを広げると no-prior shareは `0.1053` へ改善したが、chronological best pooled AUCは `0.5922` 程度でpolicy化には弱い。stress診断はvalidation evidenceとして使わない。
...
- pre-block side-gap quantileはfresh supportを戻すが、refit2025 tailも戻す。次はpre-block support normalizationの後に、新規admitted rowsだけをreplacement/tail riskで二段階審査する。
...
- pre-block `sg95` も単独では標準化しない。fresh 0-trade修正は達成したがrefit tailが大きいため、support回復とrisk admissionを分離する。
- pre-block追加refit rowsは `short/down_normal_vol` と2025-05/04のonly-candidate tailに悪化が集中した。これはstatic blacklistではなく、newly admitted rows向けの candidate-only downside / replacement-stateful-net / high-score losing-tail targetへ戻す。
...
- fresh support不足はraw row数だけでなくepisode数とactive month数で見る。q99/rank90/floor5は26 rowsでも6 episodes/1 active month/1 tradeに集中するため、標準gateには弱い。
...
- executable EV補正をpost-selector / blocked-side scoreへ後段適用すると、score scaleは縮むが良いtradeも消えて悪い少数tradeが残る。capture factorはselector前のbase calibrated scoreへ入れ、その後にexit-regret selectorとside-gap quantileを再計算する。
- selector前capture補正はpost-selector補正より改善するが、単独では2025-12 short tailを止められない。次はcapture-adjusted scoreに side/regime tail-risk head、direction-side robustness head、またはresidual `short/down_high_vol/rollover` を説明する低容量featureを重ねる。
