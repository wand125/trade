...
1. direction/regime別 calibration と予測EVのshrinkageを追加する。
2. hard gateではなく、regime別 `entry_threshold_offset` やsoft NoTrade化をvalidation内OOFで決める。
3. profit barrierを0/1予測ではなく確率として保存し、閾値をcalibrateする。
4. train期間にもOOF predictionsを作り、meta model/calibrationの学習量を増やす。
