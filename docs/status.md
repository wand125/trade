...
1. 失敗trade analyzerを標準診断にし、今後の候補へ必ず適用する。
...
5. train期間にもOOF predictionsを作り、meta model/calibrationの学習量を増やす。
6. direction/regime別 calibration と予測EVのshrinkageを追加する。
7. exit timing targetを fixed horizon、barrier time、hazard-like close probability へ拡張する。
8. shared representationを持つ小型MLP/TCNでmulti-task学習を試す。
9. 追加holdout月を用意し、2024-12/2025-02に過剰適合しない評価へ移る。
