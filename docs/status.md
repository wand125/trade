...
1. train期間にもOOF predictionsを作り、meta model/calibrationの学習量を増やす。
...
4. calibration採用基準にentry数上限、fold間trade分布、regime別direction accuracyを追加する。
5. hard gateではなく、regime別 `entry_threshold_offset` やsoft NoTrade化をvalidation内OOFで決める。
