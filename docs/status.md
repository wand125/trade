...
3. 新target入りdatasetをvalidation 4fold対象月へ再生成し、`pred_*_exit_event_minutes` をholding columnにした `timed_ev` を従来holding targetと比較する。
4. 多クラス `exit_event` のprobability出力を追加し、profit/loss/time確率をgateまたはpenaltyとして使えるか検証する。
...
7. PnL, trade count, side/session loss, short share, smoothed barrier miss, cost-aware成績を同時に満たす候補を固定する。
