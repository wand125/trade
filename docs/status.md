...
1. profit barrierを0/1予測ではなく確率として保存し、entry条件へcalibrateして使う。
2. `asia / range / low_vol` のshort大損を、追加side marginまたはentry thresholdで抑制する。
3. exit timing targetを fixed horizon、barrier time、hazard-like close probability へ拡張する。
4. candidate selectionへ、predicted profit barrier hitが0のtrade比率とside/regime別損失集中を追加する。
