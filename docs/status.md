...
12. event probabilityで予定保有時間を短縮するhazard-like policyを実装し、entry score penaltyとholding cap付きpolicyに対して比較する。
