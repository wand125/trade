...
profit barrier probability列と閾値gateを追加済み。2025-03 blindでは損失を `-49.7004` から `-29.5462` へ縮めたが、NoTradeには届かないため採用しない。最大損失は引き続き `asia / range / low_vol` のshortで、次はside-specific regime suppressionを優先する。
...
1. `asia / range / low_vol` のshort大損を、side-specific追加marginまたはblockで抑制する。
2. exit timing targetを fixed horizon、barrier time、hazard-like close probability へ拡張する。
3. candidate selectionへ、predicted/actual profit barrier miss率とside/regime別損失集中を追加する。
4. profit barrier probabilityのcalibrationを継続する。ただし2025-03 blindを見た後の閾値採用は禁止する。
5. 2025-04以降もblind holdoutとして追加し、2025-03単月だけに反応しすぎない。
