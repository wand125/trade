...
side/regime EV penaltyを追加済み。`--side-ev-penalty-rules` は `side:column=value+...:penalty` 形式で、side選択前にmatching sideのEVを直接減点する。HGB entry/side + MLP exit hybridのlocal gridでは `long:session_regime=ny_late:15` がvalidation min pnlをruleなし `81.5352` から `93.8904` へ上げた。near-top riskでは同じ減点15 + `min_entry_rank=0.5` がtopになり、2024-12反証月はruleなし `-54.6032` に対して `-5.4938` まで改善した。cost stressでも高コスト条件のbaseline `-76.3910` に対してrisk top `-26.0816` まで損失を縮めた。ただし標準条件・高コスト条件ともNoTrade `0` には届かず、標準policyへは昇格しない。探索軸として継続し、次は別holdout月を生成して固定候補を再検証する。詳細は `docs/reports/00067_2026-06-28_side_regime_ev_penalty.md` と `docs/reports/00068_2026-06-28_side_ev_penalty_cost_stress.md`。
...
18. `long:ny_late:15` side EV penalty risk topは2024-12 cost stressでbaselineより壊れにくいがNoTrade未達。次は `policy_combined` datasetの別holdout月を追加し、HGB entry/side + MLP exit hybrid predictionを生成して固定候補を再検証する。
