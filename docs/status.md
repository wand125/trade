...
14. 次はshared MLPを単純に長く回すより、entry/side calibrationとclassification hybridを切り分ける。`pred_*_exit_event_minutes` は有望なので、exit timing専用モデルとentry/sideモデルを分ける案を検証する。
