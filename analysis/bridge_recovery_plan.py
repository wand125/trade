...
HISTORY_REQUEST_COMMAND = "python3 bridge/request_history.py 168"
...
def build_recovery_actions(status: str, checks: dict[str, Any]) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if status == "needs_bridge_status":
        ...
        steps.append(manual_step("bridge", "Start or restart python3 bridge/mt5_ai_bridge.py, then verify /health and /config."))
    elif status == "needs_bridge_process":
        steps.append(manual_step("bridge", "Start python3 bridge/mt5_ai_bridge.py before waiting for MT5 EA history responses."))
    ...
    if status in {"ready", "needs_history_status_refresh", "needs_history_refresh"}:
        commands.append(
            command_step(
                ...
                "python3 analysis/mt5_back_forward_run.py --mode both --run-archive-preview "
        ...
        return [
            {
                **base,
                ...
                "target": "bridge/mt5_ai_bridge.py",
        ...
        return [
            {
                **base,
                ...
                "target": "bridge/mt5_ai_bridge.py",
                "operator_step": "Start python3 bridge/mt5_ai_bridge.py.",
...
def operator_handoff_lines(plan: dict[str, Any], checks: dict[str, Any]) -> list[str]:
    ...
    if status == "needs_ea_restart":
        ...
        lines.append("- Start `python3 bridge/mt5_ai_bridge.py`, then refresh Bridge status.")
    elif status == "needs_bridge_http":
